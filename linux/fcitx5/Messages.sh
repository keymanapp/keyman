...
source_files=$(find . -path ./build -prune -false -o -name \*.cpp -o -name \*.h)
$XGETTEXT --keyword=_ --keyword=N_ --language=C++ -o ${POT_FILE} $source_files
desktop_files=$(find . -path ./build -prune -false -o -name \*.conf.in -o -name \*.conf.in.in -o -name \*.desktop.in -o -name \*.desktop.in.in)
$XGETTEXT --language=Desktop $desktop_files -j -o ${POT_FILE}
metainfo_files=$(find  . -path ./build -prune -false -o -name \*.metainfo.xml.in)
$XGETTEXT --package-name=$DOMAIN -j -o ${POT_FILE} $metainfo_files
