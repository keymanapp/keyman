...
struct _StatusWindow
{
  ...
  /* Currently focused GtkIMContextKMFL for the toplevel, if any */
  GtkIMContextKMFL *context;
};

static void     gtk_im_context_xim_class_init         (GtkIMContextKMFLClass  *class);
static void     gtk_im_context_xim_init               (GtkIMContextKMFL       *im_context_xim);
...
static void reinitialize_ic      (GtkIMContextKMFL *context_xim);
static void set_ic_client_window (GtkIMContextKMFL *context_xim,
				  GdkWindow       *client_window);

static void setup_styles (GtkXIMInfo *info);

static void update_client_widget   (GtkIMContextKMFL *context_xim);
static void update_status_window   (GtkIMContextKMFL *context_xim);
...
static XIC       gtk_im_context_xim_get_ic            (GtkIMContextKMFL *context_xim);
...
gtk_im_context_xim_register_type (GTypeModule *type_module)
{
  ...
  {
    sizeof (GtkIMContextKMFLClass),
    (GBaseInitFunc) NULL,
    (GBaseFinalizeFunc) NULL,
    (GClassInitFunc) gtk_im_context_xim_class_init,
    NULL,           /* class_finalize */    
    NULL,           /* class_data */
    sizeof (GtkIMContextKMFL),
    0,
...
static void
gtk_im_context_xim_class_init (GtkIMContextKMFLClass *class)
{
...
static void
gtk_im_context_xim_init (GtkIMContextKMFL *im_context_xim)
{
...
{
  GtkIMContextKMFL *context_xim = GTK_IM_CONTEXT_XIM (obj);
...
static void
reinitialize_ic (GtkIMContextKMFL *context_xim)
{
  if (context_xim->ic)
    {
...
static void
set_ic_client_window (GtkIMContextKMFL *context_xim,
		      GdkWindow       *client_window)
{
...
gtk_im_context_xim_set_client_window (GtkIMContext          *context,
				      GdkWindow             *client_window)
{
  GtkIMContextKMFL *context_xim = GTK_IM_CONTEXT_XIM (context);
...
static char *
mb_to_utf8 (GtkIMContextKMFL *context_xim,
	    const char      *str)
{
  GError *error = NULL;
  gchar *result;
				    ...
				    GdkEventKey  *event)
{
...
#if DAR_DEBUG  
  fprintf(stderr, "DAR: filter_keypress keycode %x, state %x\n", event->hardware_keycode, event->state);
#endif
 ...
 again:
  ...
  /* For KMFL, Use delete_surrounding_context (if possible) if this was a backspace keyevent 
     This accomplishes two things, first context_xim_reset is not called in response
     to a backspace and, secondly, for a (Unicode) combining diacritic, backspace will
     only delete the diacritic, not the whole character cluster */
  if (keysym == XK_BackSpace) 
      if (gtk_im_context_delete_surrounding(context,-1, 1))
    	return TRUE;
...
gtk_im_context_xim_reset (GtkIMContext *context)
{
  ...
  fprintf(stderr, "DAR: gtk_im_context_xim_reset called\n");
...
#if DAR_DEBUG  
  fprintf(stderr, "DAR: context_xim->preedit_length=%d\n", context_xim->preedit_length);
#endif
  
  if (context_xim->preedit_length == 0) 
    {
      /* DAR Added to reset context even if the preedit length is 0...for IMs that do not use a preedit mode */
...
static gint
xim_text_to_utf8 (GtkIMContextKMFL *context, XIMText *xim_text, gchar **text)
{
  gint text_length = 0;
  GError *error = NULL;
  gchar *result = NULL;
...
{
  GtkIMContextKMFL *context = GTK_IM_CONTEXT_XIM (client_data);
...
set_preedit_callback (GtkIMContextKMFL *context_xim)
{
...
static XVaNestedList
set_status_callback (GtkIMContextKMFL *context_xim)
{
...
static XIC
gtk_im_context_xim_get_ic (GtkIMContextKMFL *context_xim)
{
 ...
 * for it; whenever a new GtkIMContextKMFL claims the status
...
*/
static void
disclaim_status_window (GtkIMContextKMFL *context_xim)
{
 ...
 */
static void
claim_status_window (GtkIMContextKMFL *context_xim)
{
 ...
 */
static void
update_status_window (GtkIMContextKMFL *context_xim)
{
 ...
 */
static void
update_in_toplevel (GtkIMContextKMFL *context_xim)
{
 ...
 */
static void
update_client_widget (GtkIMContextKMFL *context_xim)
{
