...
ibus_keyman_engine_init(IBusKeymanEngine *keyman) {
  ...
  keyman->state = NULL;
  keyman->display = gdk_display_get_default();
#ifdef GDK_WINDOWING_X11
  keyman->xdisplay = NULL;
  if (GDK_IS_X11_DISPLAY(keyman->display)) {
    keyman->xdisplay = GDK_DISPLAY_XDISPLAY(keyman->display);
  } else
#endif
  {
...
ibus_keyman_engine_destroy(IBusKeymanEngine *keyman) {
  ...
  gdk_display_close(keyman->display);
  IBUS_OBJECT_CLASS(parent_class)->destroy((IBusObject *)keyman);
}
