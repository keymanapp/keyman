...
static void     km_g_test_tap_print                (unsigned    subtest_level,
                                                 gboolean    commented,
                                                 const char *format,
                                                 ...) G_GNUC_PRINTF (3, 4);
...
static void
km_g_test_print_handler_full (const gchar *string,
...
static void
km_g_test_print_handler (const gchar *string)
{
  km_g_test_print_handler_full (string, test_tap_log, TRUE, is_subtest () ? 1 : 0);
}

static void
km_g_test_tap_print (unsigned    subtest_level,
                  gboolean    commented,
                  const char *format,
                  ...)
{
  ...
  km_g_test_print_handler_full (string, TRUE, commented, subtest_level);
  g_free (string);
}
  ...
  if (test_debug_log)
    {
      ...
      g_string_printf (output, "{*LOG(%s)", km_g_test_log_type_name (msg->log_type));
            ...
            long double *largs)
{
  ...
  if (g_once_init_enter_pointer (&g_default_print_func))
    {
      g_once_init_leave_pointer (&g_default_print_func,
                                 g_set_print_handler (km_g_test_print_handler));
  ...
  switch (lbit)
      ...
      if (test_tap_log)
        {
          if (!is_subtest ())
            {
              km_g_test_tap_print (0, FALSE, "TAP version " TAP_VERSION "\n");
            }
          else
            {
              km_g_test_tap_print (subtest_level > 0 ? subtest_level - 1 : 0, TRUE,
                                "Subtest: %s\n", test_argv0);
            }

          g_print ("random seed: %s\n", string2);
        }
      else if (km_g_test_verbose ())
        {
      ...
      if (test_tap_log)
        {
          ...
          if (string1[0] != 0)
            ...
            km_g_test_tap_print (subtest_level, FALSE, "1..%d\n", test_count);
      }
    ...
    case G_TEST_LOG_STOP_CASE:
      ...
      else if (!km_g_test_quiet () && !test_in_subprocess)
        g_print ("%s\n", km_g_test_result_names[result]);
      if (fail && test_mode_fatal)
        {
          if (test_tap_log)
            km_g_test_tap_print (0, FALSE, "Bail out!\n");
          g_abort ();
        }
    ...
    case G_TEST_LOG_SKIP_CASE:
      if (test_tap_log)
        {
          km_g_test_tap_print (subtest_level, FALSE, "ok %d %s # SKIP\n",
      ...
      if (test_tap_log)
        {
          ...
          if (test_run_name && *test_run_name != '\0')
            {
              if (message && *message != '\0')
                km_g_test_tap_print (subtest_level, FALSE, "not ok %s - %s\n",
                                  test_run_name, message);
              else
                km_g_test_tap_print (subtest_level, FALSE, "not ok %s\n",
                                  test_run_name);
          ...
          if (message && *message != '\0')
            km_g_test_tap_print (subtest_level, FALSE, "Bail out! %s\n", message);
          else
            km_g_test_tap_print (subtest_level, FALSE, "Bail out!\n");
  ...
  switch (lbit)
    ...
    case G_TEST_LOG_START_CASE:
      ...
      {
        g_print ("##teamcity[testStarted name='%s']\n", string1);
      }
      else if (km_g_test_verbose ())
        g_print ("GTest: run: %s\n", string1);
...
/**
 * km_g_test_disable_crash_reporting:
 *
 ...
 */
void
km_g_test_disable_crash_reporting (void)
{
#ifdef HAVE_SYS_RESOURCE_H
 ...
 *
 * This should be called before calling any other `km_g_test_*()` functions.
 *
 ...
 *
 * - `G_TEST_OPTION_NO_PRGNAME`: Causes km_g_test_init() to not call
 ...
 *
 * Since 2.58, if tests are compiled with `G_DISABLE_ASSERT` defined, `km_g_test_init()`
               ...
               ...)
{
  ...
  g_return_if_fail (km_g_test_config_vars->test_initialized == FALSE);
...
km_g_test_case_free (GTestCase *test_case)
{
   ...
   * passed ownership of it into km_g_test_add_data_func_full() with a
   * #GDestroyNotify. */
  ...
  if (echo_stdout && test_tap_log && data.stdout_str->len > 0)
    {
      ...
      km_g_test_print_handler_full (data.stdout_str->str, TRUE, TRUE, 1);
 ...
 */
void
km_g_test_trap_subprocess (const char           *test_path,
                        guint64               usec_timeout,
                        GTestSubprocessFlags  test_flags)
{
  km_g_test_trap_subprocess_with_envp (test_path, NULL, usec_timeout, test_flags);
}

/**
 * km_g_test_trap_subprocess_with_envp:
 * @test_path: (nullable): test to run in a subprocess
