...
{  //----------------------------------------
  ...
  // check Paramtr:
  std::wcout << L"arc:   " << argc   << L"\n";
  for ( int i=0; i<argc-1 ; i++) {
    std::wcout << L"arg["<<i<<"]: " << argv[i]   << L"\n";
  }
...
*/
  ...
  LPKEYBOARD kmxfile;
std::wcout << L" before LoadKbd: \n"; 
std::wcout << L" before LoadKbd: " << infile_c << L".." << *infile_c <<L"\n\n"; 
  if(!LoadKeyboard(infile, &kmxfile)) {
    //MyCout("TODO: Replace: Failed to load keyboard (%d), GetLastError());",1);          //LogError(L"Failed to load keyboard (%d)", GetLastError());
    return 3;
  }
...
status mc_kmx-file.cpp:
  fopen-why does it not work
  find replacement: DebugLog
  find replacement: VerifyKeyboard
  find replacement: CopyKeyboard
  find replacement: FixupKeyboard
  wchar<->char
  char16_t<->wchar_t
*/
