...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="menuframe">
    <script type="text/javascript">
          ...
          //menuframe_add('<xsl:value-of select="/Keyman/templatepath"/>', 'keepintouch', '<xsl:value-of select="$locale/String[@Id='S_KeepInTouch']"/>', '<xsl:value-of select="$locale/String[@Id='S_KeepInTouch_AccessChar']"/>');
    </script>
