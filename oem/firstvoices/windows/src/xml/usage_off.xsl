			...
			<head>
				...
				<style type="text/css">
					* { font-family: <xsl:value-of select="($locale/String[@Id='SK_UIFontName'])[1]" />; font-size: 13.3px; }
					...
					body { padding: 6px; margin: 0px; overflow-y: auto;
					background: #A0C1DC; border: none; }
					html { padding: 0px; margin: 0px; overflow-y: auto; }
					...
					#inner { position: relative; top: -50%; width: 100%; font-size: 13.3px; text-align: center; }
					
					#nokeyboards { width: 90%; padding: 8px; background: #F5F6BE; border: solid 1px gray; text-align: left }
