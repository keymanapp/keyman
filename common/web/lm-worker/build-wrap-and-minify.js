...
console.log(`JSON.stringify time taken: ${end-start}`);
console.log(`JSON.stringify size: ${jsonEncoded.length}`);

let start3 = performance.now();
let jsonParsed = JSON.parse(jsonEncoded);
let end3 = performance.now();
console.log(`JSON.parse time taken: ${end3-start3}`);

// Two layers of encoding:  one for the raw source (parsed by the JS engine),
// one to 'unwrap' it from a string _within_ that source.
let jsonDoubleEncoded = JSON.stringify(jsonEncoded);

let start2 = performance.now();
let uriEncoded = encodeURIComponent(rawScript);
let end2 = performance.now();
console.log(`encodeURIComponent time taken: ${end2-start2}`);
console.log(`encodeURIComponent size: ${uriEncoded.length}`);

let start4 = performance.now();
decodeURIComponent(uriEncoded);
let end4 = performance.now();
console.log(`decodeURIComponent time taken: ${end4-start4}`);
