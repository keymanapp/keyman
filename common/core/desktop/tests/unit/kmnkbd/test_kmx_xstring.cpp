...
void test_incxstr() {
  ...
  PKMX_WCHAR q;   // pointer output to incxstr()
    ...
    // --- Test for FFFF +sequence (earlier p+1) with \0 after first position ----- unit test of incxstr() failed before ---------------------------
    p = (PKMX_WCHAR)u"\U0000FFFF\0\U00000008\U00000001";
    q = incxstr(p);
    assert(q == p+1);

    // --- Test for FFFF +sequence (earlier p+1) with \0 after second position ---- unit test of incxstr() failed before ---------------------------
    p = (PKMX_WCHAR)u"\U0000FFFF\U00000008\0\U00000001";
    q = incxstr(p);
    assert(q == p+2);

    // --- Test for FFFF +sequence (earlier p+1) with \0 after third position ----- unit test of incxstr() failed before ---------------------------
    p = (PKMX_WCHAR)u"\U0000FFFF\U00000008\U00000001\0";
    q = incxstr(p);
    assert(q == p+3);
                          

    // --- Test for FFFF +sequence (earlier p+2) with \0 after fourth position ----- unit test of incxstr() failed before --------------------------
    p = (PKMX_WCHAR)u"\U0000FFFF\U00000002\U00000001\U00000001\0";
    q = incxstr(p);
    assert(q == p+4);


    // --- Test for FFFF +sequence (earlier p+3) with \0 after fifth  position ----- unit test of incxstr() failed before --------------------------
    p = (PKMX_WCHAR)u"\U0000FFFF\U00000014\U00000001\U00000001\U00000001\0";
    q = incxstr(p);
    assert(q == p+5);
 
    // --- Test for FFFF +Sequence CODE_EXTENDED ----- (earlier p+n) with \0 after 6.  position  ----- unit test of incxstr() failed before -------- 
    p = (PKMX_WCHAR)u"\U0000FFFF\U0000000A\U00000001\U00000002\U00000003\U00000004\0\U00000005\U00000006\U00000007\U00000010";
    q = incxstr(p);
    assert(q == p + 6);

    // --- Test for FFFF +Sequence CODE_EXTENDED ----- (earlier p+n) with \0 after 7.  position  ----- unit test of incxstr() failed before --------
    p = (PKMX_WCHAR)u"\U0000FFFF\U0000000A\U00000001\U00000002\U00000003\U00000004\U00000005\0\U00000006\U00000007\U00000010";
    q = incxstr(p);
    assert(q == p + 7);

    // --- Test for FFFF +Sequence CODE_EXTENDED ----- (earlier p+n) with \0 after 8.  position  ----- unit test of incxstr() failed before --------
    p = (PKMX_WCHAR)u"\U0000FFFF\U0000000A\U00000001\U00000002\U00000003\U00000004\U00000005\U00000006\0\U00000007\U00000010";
    q = incxstr(p);
    assert(q == p + 8);

    // --- Test for FFFF +Sequence CODE_EXTENDED ----- (earlier p+n) with \0 after 9.  position  ----- unit test of incxstr() failed before ---------
    p = (PKMX_WCHAR)u"\U0000FFFF\U0000000A\U00000001\U00000002\U00000003\U00000004\U00000005\U00000006\U00000007\0\U00000010";
    q = incxstr(p);
    assert(q == p + 9);

    // --- Test for FFFF +Sequence CODE_EXTENDED ----- (earlier p+n) with \0 after 10.  position  ----- unit test of incxstr() failed before --------
