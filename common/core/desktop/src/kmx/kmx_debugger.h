...
inline KMX_DebugItems::KMX_DebugItems(debug_items *items) {
  assert(items);
  _items = items;
}

inline
void KMX_DebugItems::push_group_enter(LPGROUP group) {
  push_item(KM_KBP_DEBUG_GROUP_ENTER, 0, group, NULL, NULL);
}

inline
void KMX_DebugItems::push_group_exit(LPGROUP group, uint32_t flags) {
  push_item(KM_KBP_DEBUG_GROUP_EXIT, flags, group, NULL, NULL);
}

inline
void KMX_DebugItems::push_nomatch_enter(LPGROUP group) {
  push_item(KM_KBP_DEBUG_NOMATCH_ENTER, 0, group, NULL, NULL);
}

inline
void KMX_DebugItems::push_nomatch_exit(LPGROUP group) {
  push_item(KM_KBP_DEBUG_NOMATCH_EXIT, 0, group, NULL, NULL);
}

inline
void KMX_DebugItems::push_match_enter(LPGROUP group) {
  push_item(KM_KBP_DEBUG_MATCH_ENTER, 0, group, NULL, NULL);
}

inline
void KMX_DebugItems::push_match_exit(LPGROUP group) {
  push_item(KM_KBP_DEBUG_MATCH_EXIT, 0, group, NULL, NULL);
}

inline
void KMX_DebugItems::push_rule_enter(LPGROUP group, LPKEY key, PKMX_WCHAR context) {
  push_item(KM_KBP_DEBUG_RULE_ENTER, 0, group, key, context);
}

inline
void KMX_DebugItems::push_rule_exit(LPGROUP group, LPKEY key, PKMX_WCHAR context) {
  push_item(KM_KBP_DEBUG_RULE_EXIT, 0, group, key, context);
}

inline
void KMX_DebugItems::push_begin(km_kbp_state_debug_key_info *key_info, uint32_t flags) {
...
inline
void KMX_DebugItems::push_end(uint32_t flags) {
