...
public:
    ...
    static uchar_t get(const codeunit_t * cp, int8_t & l) throw()
    {
        ...
        bool toolong = false;
        switch(seq_sz) {
            case 4:     u <<= 6; u |= *++cp & 0x3F; if (*cp >> 6 != 2) break; --l; ++rl; toolong  = (u < 0x10); fallthrough;
            case 3:     u <<= 6; u |= *++cp & 0x3F; if (*cp >> 6 != 2) break; --l; ++rl; toolong |= (u < 0x20); fallthrough;
            case 2:     u <<= 6; u |= *++cp & 0x3F; if (*cp >> 6 != 2) break; --l; ++rl; toolong |= (u < 0x80); fallthrough;
            case 1:     l = seq_sz; ++rl; break;
            case 0:     l = -1; return 0xFFFD;
        }

        if (rl != seq_sz || toolong  || u >= limit || l < 0)
        {
            l = -rl;
            return 0xFFFD;
        }
