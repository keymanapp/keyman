...
function do_build() {
  ...
  export_apps
  create_packages
  build_distribution_xml_from_template
  combine_packages
...
function build_distribution_xml_from_template(){
  echo "Created distribution.xml from template at ${OUTPUT__DISTRIBUTION_XML}"
  export KEYMAN_VERSION
  envsubst \$KEYMAN_VERSION < distribution_template.xml > ${OUTPUT__DISTRIBUTION_XML}
}
