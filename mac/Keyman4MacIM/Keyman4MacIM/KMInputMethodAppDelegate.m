    ...
    switch (type) {
      ...
      case kCGEventKeyDown:
        ...
        if(sysEvent.keyCode == 255) {
          ...
          os_log_debug([KMLogs eventsLog], "*** other: %d(0x%X)", (char) sysEvent.keyCode, sysEvent.keyCode);
          
          // check to see if this event was generated by the OSK
          int64_t OSK_EVENT_FLAG = 0x8000000000000000;
          int64_t OSK_EVENT_MODIFIER_MASK = 0x00000000FFFFFFFF;
          int64_t eventUserData = CGEventGetIntegerValueField(event, kCGEventSourceUserData);
          if ((eventUserData & OSK_EVENT_FLAG) == OSK_EVENT_FLAG) {
            NSEventModifierFlags keyDownModifiers = (NSEventModifierFlags) eventUserData & OSK_EVENT_MODIFIER_MASK;
            os_log_debug([KMLogs eventsLog], "*** keydown event generated by OSK, UserData: 0x%llX Modifiers: 0x%lX", eventUserData, (unsigned long)keyDownModifiers);
            appDelegate.receivedKeyDownFromOsk = YES;
            appDelegate.oskEventModifiers = keyDownModifiers;
          } else {
            os_log_debug([KMLogs eventsLog], "*** keydown event not generated by OSK, UserData = 0x%llX", eventUserData);
          }
        }
