...
write_nfd_table() {
  ...
  FILE *f = fopen(NFD_FILE, "w");
  assert(f != nullptr);
  // we're going to need an NFD normalizer
  UErrorCode status           = U_ZERO_ERROR;
  const icu::Normalizer2 *nfd = icu::Normalizer2::getNFDInstance(status);
  assert(U_SUCCESS(status));
  fprintf(f, "// NFD hasBoundaryBefore\n");
  fprintf(f, "// Generated by test_transforms.cpp\n");
  fprintf(f, "// regenerate with: core/build/SOMETHING/SOMETHING/tests/unit/ldml/test_transforms --write-nfd\n");
  fprintf(f, "export const ICU_VERSION='%s';\n", U_ICU_VERSION);
  ...
  fprintf(f, "];\n\n");
  fclose(f);
  std::cout << " wrote: " << NFD_FILE << std::endl;
  return 0;
}
...
main(int argc, const char *argv[]) {
  ...
  if (first_arg < argc) {
    auto arg_nfd = std::string(argv[first_arg]) == "--write-nfd";
    if (arg_nfd) {
      first_arg++;
      if (write_nfd_table() != EXIT_SUCCESS) {
        rc = EXIT_FAILURE;
      }
      return rc;
    }
