 ...
 *
 * Created by Ross Cruickshank on 2018-10-30
 *
...
/* This test will test the infrastructure around IMX third party libraries
...
uint8_t test_imx_callback(km_core_state *state, uint32_t imx_id, void *callback_object) {
  ...
  size_t n = 0;
  if(context_items_to_utf16(entry_context, nullptr, &n) != KM_CORE_STATUS_OK) return FALSE;
  km_core_cu *buf = new km_core_cu[n];
  if(context_items_to_utf16(entry_context, buf, &n) != KM_CORE_STATUS_OK) return FALSE;
  ...
  n = 0;
  if(context_items_to_utf16(exit_context, nullptr, &n) != KM_CORE_STATUS_OK) return FALSE;
  km_core_cu *tmp_buf = new km_core_cu[n];
  if(context_items_to_utf16(exit_context, tmp_buf, &n) != KM_CORE_STATUS_OK) return FALSE;
...
//void test_imx_list(const km::core::path &source_file){
  ...
  km::core::path full_path = km::core::path(test_dir / "kmx_imsample.kmx");
  ...
  ASSERT_STATUS_OK(km_core_keyboard_get_imx_list(test_kb, &kb_imx_list));
  ...
  for (; imx_rule_it->library_name; ++imx_rule_it) {
    ...
    ASSERT_EQ(extracted_library_function, expected_imx_map[imx_rule_it->imx_id] );
...
TEST(ImxTests, TestQueueActions) {
  ...
  ASSERT_TRUE(km::tests::action_items(test_state, {{KM_CORE_IT_CHAR, {0,}, {km_core_usv('X')}}, {KM_CORE_IT_ALERT, {0,}, {0}}, {KM_CORE_IT_END}}));

  ASSERT_STATUS_OK(km_core_process_event(test_state, KM_CORE_VKEY_ESC, 0, 1, KM_CORE_EVENT_FLAG_DEFAULT));
  ASSERT_TRUE(km::tests::action_items(test_state, { {KM_CORE_IT_CHAR, {0,}, {km_core_usv('Y')}},
  ...
  ASSERT_TRUE(km::tests::action_items(test_state, {bksp_a,{KM_CORE_IT_CHAR, {0,}, {km_core_usv('Z')}}, {KM_CORE_IT_END}}));

  ASSERT_STATUS_OK(km_core_process_event(test_state, KM_CORE_VKEY_2, 0, 1, KM_CORE_EVENT_FLAG_DEFAULT));
  ASSERT_TRUE(km::tests::action_items(test_state, {{KM_CORE_IT_INVALIDATE_CONTEXT, {0,}, {0}}, {KM_CORE_IT_END}}));
