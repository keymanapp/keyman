...
TEST(XStringTests, TestDecxstr) {
    ...
    // ---------------------------------------------------------------------------------------
     ...
     for (auto code = 1 /* CODE_ANY */; code <= CODE_LASTCODE; code++) {
       ...
       p       = find_ptr_to_last_character(p_start);
       q       = decxstr(p, p_start);
       ASSERT_EQ(q, (p - 1)); // this is in the middle of the sequence so it should always go back a single unit
       ...
       if (size < 0) {
         ...
         ASSERT_EQ(q, (p - size - 2)); /* UC_SENTINEL + code + (CODE__SIZE = number of params) */
       }
     }
...
TEST(XStringTests, TestXstrlen) {
  ASSERT_EQ(xstrlen((PKMX_WCHAR)u""), 0);
  ASSERT_EQ(xstrlen((PKMX_WCHAR)u"1"), 1);
  ASSERT_EQ(xstrlen((PKMX_WCHAR)u"1234567890"), 10);
  ASSERT_EQ(xstrlen((PKMX_WCHAR) C_CODE_DEADKEY(u"\u0001") u"a"), 2);
  ASSERT_EQ(xstrlen((PKMX_WCHAR) C_CODE_IFOPT(u"\u0001", u"\u0002", u"\u0003") u"a"), 2);
  ASSERT_EQ(xstrlen((PKMX_WCHAR) C_CODE_IFSYSTEMSTORE(u"\u0001", u"\u0002", u"\u0003") u"a"), 2);
  ASSERT_EQ(xstrlen((PKMX_WCHAR) U_1F609_WINKING_FACE u"a"), 2);
}

TEST(XStringTests, TestXstrlenIgnoreifopt) {
  ASSERT_EQ(xstrlen_ignoreifopt((PKMX_WCHAR)u""), 0);
  ASSERT_EQ(xstrlen_ignoreifopt((PKMX_WCHAR)u"1"), 1);
  ASSERT_EQ(xstrlen_ignoreifopt((PKMX_WCHAR)u"1234567890"), 10);
  ASSERT_EQ(xstrlen_ignoreifopt((PKMX_WCHAR) C_CODE_DEADKEY(u"\u0001") u"a"), 2);
  ASSERT_EQ(xstrlen_ignoreifopt((PKMX_WCHAR) C_CODE_IFOPT(u"\u0001", u"\u0002", u"\u0003") u"a"), 1);
  ASSERT_EQ(xstrlen_ignoreifopt((PKMX_WCHAR) C_CODE_IFSYSTEMSTORE(u"\u0001", u"\u0002", u"\u0003") u"a"), 1);
  ASSERT_EQ(xstrlen_ignoreifopt((PKMX_WCHAR) U_1F609_WINKING_FACE u"a"), 2);
}

TEST(XStringTests, TestUtf32) {
  const KMX_DWORD u295 =     0x0127; // ħ

  ASSERT_TRUE(Uni_IsBMP(u295));

  const char16_t hmaqtugha = Uni_UTF32BMPToUTF16(u295);
  ASSERT_EQ(hmaqtugha, 0x0127);

  char16_single c0;
  int l0 = Utf32CharToUtf16(u295, c0);
  ASSERT_EQ(l0, 1);
  ASSERT_EQ(c0.ch[0], 0x0127);
  ASSERT_EQ(c0.ch[1], 0);
  ...
  ASSERT_FALSE(Uni_IsBMP(scat));
  char16_single c1;
  int l1 = Utf32CharToUtf16(scat, c1);
  ASSERT_EQ(l1, 2);
  ASSERT_EQ(c1.ch[0], 0xD83D);
  ASSERT_EQ(c1.ch[1], 0xDE40);
  ASSERT_EQ(c1.ch[2], 0);
...
TEST(XStringTests, TestU16stringToU32string) {
  // normal cases
  {
  ...
  }
  {
    ...
    ASSERT_EQ(str.at(0), 0x0065);
  }
  {
    const std::u32string str = u16string_to_u32string(u"🙀");
    ASSERT_EQ(str.length(), 1);
    ASSERT_EQ(str.at(0), 0x0001F640);
  }
  {
    const std::u32string str = u16string_to_u32string(u"Ω🙀");
    ASSERT_EQ(str.length(), 2);
    ASSERT_EQ(str.at(0), u'Ω');
    ASSERT_EQ(str.at(1), 0x0001F640);
  }
  ...
  }
  {
      ...
      ASSERT_EQ(str.length(), 2);
      ASSERT_EQ(str.at(0), 0xFFFD);
      ASSERT_EQ(str.at(1), 0xFFFD);
  }
}

TEST(XStringTests, TestU32stringToU16string) {
  // normal cases
  {
  ...
  }
  {
    const auto str = u32string_to_u16string(U"🙀");
    ASSERT_EQ(str.length(), 2);
    ASSERT_EQ(str.at(0), 0xD83D);
    ASSERT_EQ(str.at(1), 0xDE40);
  }
  {
    const auto str = u32string_to_u16string(U"Ω🙀");
    ASSERT_EQ(str.length(), 3);
    ASSERT_EQ(str.at(0), u'Ω');
    ASSERT_EQ(str.at(1), 0xD83D);
    ASSERT_EQ(str.at(2), 0xDE40);
  }
}
...
TEST(XStringTests, TestIsValid) {
 ...
 // positive range test
  ASSERT_EQ(Uni_IsValid(0x100000, 0x10FFFD), true);
  ASSERT_EQ(Uni_IsValid(0x10, 0x20), true);
  ASSERT_EQ(Uni_IsValid(0x100000, 0x10FFFD), true);

  // all valid ranges in BMP
  ASSERT_EQ(Uni_IsValid(0x0000, 0xD7FF), true);
  ASSERT_EQ(Uni_IsValid(0xD800, 0xDFFF), false);
  ASSERT_EQ(Uni_IsValid(0xE000, 0xFDCF), true);
  ASSERT_EQ(Uni_IsValid(0xFDD0, 0xFDEF), false);
  ASSERT_EQ(Uni_IsValid(0xFDF0, 0xFDFF), true);
  ASSERT_EQ(Uni_IsValid(0xFDF0, 0xFFFD), true);

  // negative range test
  ASSERT_EQ(Uni_IsValid(0, 0x10FFFF), false);         // ends with nonchar
  ASSERT_EQ(Uni_IsValid(0, 0x10FFFD), false);         // contains lots o' nonchars
  ASSERT_EQ(Uni_IsValid(0x20, 0x10), false);          // swapped
  ASSERT_EQ(Uni_IsValid(0xFDEF, 0xFDF0), false);      // just outside range
  ASSERT_EQ(Uni_IsValid(0x0000, 0x010000), false);    // crosses noncharacter plane boundary and other stuff
  ASSERT_EQ(Uni_IsValid(0x010000, 0x020000), false);  // crosses noncharacter plane boundary
  ASSERT_EQ(Uni_IsValid(0x0000, 0xFFFF), false);      // crosses other BMP prohibited and plane boundary
  ASSERT_EQ(Uni_IsValid(0x0000, 0xFFFD), false);      // crosses other BMP prohibited
  ASSERT_EQ(Uni_IsValid(0x0000, 0xE000), false);      // crosses surrogate space
  ASSERT_EQ(Uni_IsValid(0x0000, 0x20FFFF), false);      // out of bounds
  ASSERT_EQ(Uni_IsValid(0x10FFFD, 0x20FFFF), false);      // out of bounds
}
