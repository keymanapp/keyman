...
describe('KeylayoutToKmnConverter', function () {
  ...
  describe('RunOneFile ', function () {
    ...
    sut.run(inputFilename, outputFilename);
    assert.isTrue(true);
    ;
  });
  ...
  describe('RunAllFiles ', function () {
    ...
    [
      ...
      const outputFilename = "data//" + files_[0].substring(0, files_[0].lastIndexOf(".")) + ".kmn";
      sut.run(files_[0], outputFilename);
      assert.isTrue(true);
    });
  });


  //const inputFilename = makePathToFixture('../data/Italian.keylayout');              // OKOK
  //const inputFilename  = makePathToFixture('../data/Spanish.keylayout');             // OKOK
  // no const inputFilename = makePathToFixture('../data/German_complete.keylayout');           // OK NO NCAPS SHIFT <-> SHIFT NCAPS
  // no const inputFilename  = makePathToFixture('../data/German_Standard.keylayout');     // OK NO NCAPS SHIFT <-> SHIFT NCAPS
  // no const inputFilename  = makePathToFixture('../data/Latin_American.keylayout');      // OKOK
  //const inputFilename = makePathToFixture('../data/Swiss_French.keylayout');        // OKOK
  //const inputFilename  = makePathToFixture('../data/Swiss_German.keylayout');        // OKOK
  //const inputFilename  = makePathToFixture('../data/US.keylayout');                  // OKOK
  //const inputFilename = makePathToFixture('../data/Polish.keylayout');               // OKOK
  //const inputFilename = makePathToFixture('../data/French.keylayout');               // OKOK
  ...
  describe("run() ", function () {
    ...
    it('run() should throw on null input file name', async function () {
      let threw = false;
      try {
    ...
    it('run() should return on unavailable input file name and null output file name', async function () {
    ...
    it('run() throw on incorrect input file extention ', async function () {
      console.log(" ------------------",);
      const inputFilename = makePathToFixture('../data/Italian_command.A');
      const outputFilename = makePathToFixture('../../data/OutputName.B');
      let threw = false;
      try {
        await sut.run(inputFilename, outputFilename);
      } catch {
    ...
    describe("read() ", function () {
      ...
      it('read() should return empty array  on typo in path', async function () {
        const result = sut.read('../data|Italian.keylayout');
        assert.isEmpty(result);
      });
    });
