...
import { KL_KeyMapSelect } from "../../../common/web/utils/src/types/keylayout/keylayout-xml.js";
import { KL_KeyMap } from "../../../common/web/utils/src/types/keylayout/keylayout-xml.js";
...
describe('KeylayoutFileReader', function () {
  ...
  describe("validate() ", function () {
    ...
    it('validate() should return false on inputfile with unknown tags', async function () {
      const sutR = new KeylayoutFileReader(compilerTestCallbacks);
      const inputFilename = makePathToFixture('../data/tests-keylayout-kmn/Test_unknownTags.keylayout');
      const result: Keylayout.KeylayoutXMLSourceFile | null = sutR.read(compilerTestCallbacks.loadFile(inputFilename));
      const validated = sutR.validate(result as Keylayout.KeylayoutXMLSourceFile, inputFilename);
    ...
    it('validate() should return false on inputfile with additional tags', async function () {
      const sutR = new KeylayoutFileReader(compilerTestCallbacks);
      const inputFilename = makePathToFixture('../data/tests-keylayout-kmn/Test_additionalTags.keylayout');
      const result: Keylayout.KeylayoutXMLSourceFile | null = sutR.read(compilerTestCallbacks.loadFile(inputFilename));
      const validated = sutR.validate(result as Keylayout.KeylayoutXMLSourceFile, inputFilename);
      assert.isFalse(validated);
    });
    it('validate() should return false on inputfile with missing tags', async function () {
      const sutR = new KeylayoutFileReader(compilerTestCallbacks);
      const inputFilename = makePathToFixture('../data/tests-keylayout-kmn/Test_missingTags.keylayout');
      const result: Keylayout.KeylayoutXMLSourceFile | null = sutR.read(compilerTestCallbacks.loadFile(inputFilename));
      const validated = sutR.validate(result as Keylayout.KeylayoutXMLSourceFile, inputFilename);
      assert.isFalse(validated);
    });
    it('validate() should return false on no entries in action-when', async function () {
      const sutR = new KeylayoutFileReader(compilerTestCallbacks);
      const inputFilename = makePathToFixture('../data/tests-keylayout-kmn/Test_noActionWhen.keylayout');
      const result: Keylayout.KeylayoutXMLSourceFile | null = sutR.read(compilerTestCallbacks.loadFile(inputFilename));
      const validated = sutR.validate(result as Keylayout.KeylayoutXMLSourceFile, inputFilename);
      assert.isFalse(validated);
    });
  ...
  describe("read() ", function () {
    ...
    it('read() should return filled array on correct input', async function () {
      ...
      const inputFilename = makePathToFixture('../data/tests-keylayout-kmn/tests-keylayout-kmn/Test.keylayout');
    ...
    it('read() should return empty array on unavailable file name', async function () {
      const inputFilenameUnavailable = makePathToFixture('../data/tests-keylayout-kmn/tests-keylayout-kmn/X.keylayout');
      const result = sutR.read(compilerTestCallbacks.loadFile(inputFilenameUnavailable));
      assert.isNull(result);
    });
  ...
  describe('checkForCorrespondingElements ', function () {
    const sutR = new KeylayoutFileReader(compilerTestCallbacks);
    [
      ['../data/tests-keylayout-kmn/Test.keylayout', true],
      ['../data/tests-keylayout-kmn/Test_sameKeyMapAndKeyMapselectAndJisERROR.keylayout', true],
      ['../data/tests-keylayout-kmn/Test_moreKeymapSelectThanKeymapERROR.keylayout', false],
      ['../data/tests-keylayout-kmn/Test_moreKeyMapThanKeyMapselectERROR.keylayout', false],
      ['../data/tests-keylayout-kmn/Test_moreKeyMapThanKeyMapselectAndJisERROR.keylayout', false],
    ].forEach(function (values) {
