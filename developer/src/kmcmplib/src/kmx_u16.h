...
// This template can be used for  KMX_CHAR*/const KMX_CHAR*,  KMX_WCHART*/const KMX_WCHART* and  KMX_WCHAR*/const KMX_WCHAR*
...
if (!(std::is_same<T,TT>::value))
  ...
  if ( (std::is_same<T,const KMX_CHAR*>::value) || (std::is_same<T, KMX_CHAR*>::value))
    nfile = fopen((const KMX_CHAR*) Filename, (const KMX_CHAR*) mode);

  else if ( (std::is_same<T,const KMX_WCHART*>::value) ||  (std::is_same<T, KMX_WCHART*>::value))
    nfile = _wfsopen((const KMX_WCHART*) Filename, (const KMX_WCHART*) mode, _SH_DENYWR);

  else if ((std::is_same<T,const KMX_WCHAR*>::value) ||(std::is_same<T,const KMX_WCHAR*>::value))
  {
