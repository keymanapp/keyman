 ...
 * in .kmx.
 */
import { KMX, KmxFileReader, KMXPlus } from "@keymanapp/common-types";
...
export class EmbedOskInKmx {
  ...
  public embed(kmx: Uint8Array, kvksFilename: string, touchLayoutFilename: string, displayMap: PuaMap): Uint8Array {
    ...
    if(touchLayoutFilename) {
      ...
      // We expect a valid .kmx file because it was most likely generated by an
      // earlier step in this process, so allow any exceptions reading it to
      // bubble up
      const reader = new KmxFileReader();
      const keyboard = reader.read(kmx);
      const vkDictionary = keyboard.stores.find(store => store.dwSystemID == KMX.KMXFile.TSS_VKDICTIONARY)?.dpString ?? '';

      embedTouchLayout.transformTouchLayoutToKmxPlus(kmxPlus, touchLayout, vkDictionary);
    }
