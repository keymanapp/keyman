...
export class KmpCompiler implements KeymanCompiler {
  ...
  private kpsInfoToKmpInfo(kpsInfo: KpsFile.KpsFileInfo): KmpJsonFile.KmpJsonFileInfo {
    ...
    for (let [src,dst,isMarkdown] of keys) {
      if (kpsInfo[src]) {
        kmpInfo[dst] = {
          // @ts-ignore: VSCode is complaining about toString(), not sure why
  ...
  private async getFileDataLocal(kpsFilename: string, inputFilename: string): Promise<KmpCompilerFileDataResult> {
