...
begin
 case FCellStyle of
  csDefault:
   begin
    {$IFDEF DELPHI_7_UP}
    if StyleServices.Enabled then  // I3309
     begin
  ...
  csCorel:
   begin
    if Enabled then
     begin
      ...
      if StyleServices.Enabled then  // I3309
       case FState of
        ...
        ccsOver: StyleServices.DrawElement(ACanvas.Handle, StyleServices.GetElementDetails(ttbButtonHot), R);  // I3309
        ccsDown: StyleServices.DrawElement(ACanvas.Handle, StyleServices.GetElementDetails(ttbButtonPressed), R);  // I3309
        ccsChecked: StyleServices.DrawElement(ACanvas.Handle, StyleServices.GetElementDetails(ttbButtonChecked), R); // I3309
        ccsCheckedHover: StyleServices.DrawElement(ACanvas.Handle, StyleServices.GetElementDetails(ttbButtonCheckedHot), R); // I3309
       end
      else
      {$ENDIF}
       case FState of
     ...
     {$IFDEF DELPHI_7_UP}
     if StyleServices.Enabled then // I3309
      StyleServices.DrawElement(ACanvas.Handle, StyleServices.GetElementDetails(ttbButtonDisabled), R)
     else
     {$ENDIF}
      begin
