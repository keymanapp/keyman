...
$('#btnEditPlatform').click(function () {
  ($('#chkDisplayUnderlying')[0] as HTMLInputElement).checked = KVKL[builder.lastPlatform].displayUnderlying;
...
$('#chkDisplayUnderlying').click(function (event) {
  ...
  let selection = builder.saveSelection();
  builder.saveUndo();
  KVKL[builder.lastPlatform].displayUnderlying = ($('#chkDisplayUnderlying')[0] as HTMLInputElement).checked;
  builder.generate();
