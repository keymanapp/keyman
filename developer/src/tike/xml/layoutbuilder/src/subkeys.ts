...
builder.delSelectedSubKey = function () {
...
builder.selectedSubKey = function () {
  return $('#sub-key-groups .selected');
};

builder.generateSubKeys = function () {
...
builder.prepareSubKey = function () {
...
const subKeyNameChange = builder.wrapChange(function (evt: JQueryEventObject) {
  var nkey = builder.selectedSubKey();
  if (nkey.length == 0) return;
  nkey.data('id', $(evt.currentTarget).val());
  builder.generateSubKeys();
...
builder.subKeyCapChange = function(val) {
...
const inpSubKeyCapChange = builder.wrapChange(function (evt: JQueryEventObject) {
  let val = $(evt.currentTarget).val();
  $('#inpSubKeyCapUnicode').val(builder.toUnicodeString(val));
  builder.subKeyCapChange(val);
}, {saveOnce: true});

const inpSubKeyCapUnicodeChange = builder.wrapChange(function (evt: JQueryEventObject) {
...
const selSubKeyCapTypeChange = builder.wrapChange(function (evt: JQueryEventObject) {
  evt.stopPropagation();
...
const selSubKeyNextLayerChange = builder.wrapChange(function (evt: JQueryEventObject) {
  $(evt.currentTarget).val() === '' ?
    builder.selectedSubKey().removeData('nextlayer') :
    builder.selectedSubKey().data('nextlayer', $(evt.currentTarget).val());
  builder.generateSubKeys();
});

$('#selSubKeyNextLayer').change(selSubKeyNextLayerChange);

const chkSubKeyIsDefaultChange = builder.wrapChange(function (evt: JQueryEventObject) {
...
const selSubKeyLayerOverrideChange = builder.wrapChange(function (evt: JQueryEventObject) {
  $(evt.currentTarget).val() === '' ?
    builder.selectedSubKey().removeData('layer') :
    builder.selectedSubKey().data('layer', $(evt.currentTarget).val());
...
const selSubKeyTypeChange = builder.wrapChange(function (evt: JQueryEventObject) {
  var sp = $(evt.currentTarget).val(), key = builder.selectedSubKey();
  if (key.length == 0) return;
  if (sp == 0) {
    key.removeData('sp');
  } else {
    key.data('sp', $(evt.currentTarget).val());
  }
  builder.formatKey(key, $(evt.currentTarget).val());
  builder.generateSubKeys();
});
...
builder.selectSubKey = function (key) {
...
builder.enableSubKeyControls = function () {
  var key = builder.selectedSubKey();
  if (key.length == 0) {
