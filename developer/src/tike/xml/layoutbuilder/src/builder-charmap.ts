...
builder.dragDrop = {
  isSubKey: function(key) {
    return key.data('type') != 'key';
  },

  getKeyElementFromElement: function(elem) {
  ...
  getKeyElementFromDragTarget: function(target) {
    let key = builder.dragDrop.getKeyElementFromElement(target);
    let fieldType = 'text';
  ...
  clearKeySelectTimeout: function() {
    if(builder.dragDrop.keySelectTimeout) {
      window.clearTimeout(builder.dragDrop.keySelectTimeout);
      builder.dragDrop.keySelectTimeout = 0;
    }
  },
};
...
builder.charmapDragOver = function(o) {
  ...
  builder.dragDrop.clearKeySelectTimeout();

  const target = document.elementFromPoint(o.x, o.y);
  const key = target ? builder.dragDrop.getKeyElementFromDragTarget(target) : null;
  const found = key && key.key && key.key.length;

  builder.dragDrop.keySelectTimeout = window.setTimeout(function() {
    ...
    } else if(builder.dragDrop.isSubKey(key.key)) {
...
builder.charmapDragDrop = function(o) {
  ...
  if(o.x >= 0 && o.y >= 0) {
    ...
    if(target != null) {
      key = builder.dragDrop.getKeyElementFromDragTarget(target);

      if(key && key.key && key.key.length) {
        if(builder.dragDrop.isSubKey(key.key)) {
          builder.selectSubKey(key.key);
        } else {
          builder.selectKey(key.key);
        }
  ...
  const target = $(
    ...
    builder.dragDrop.isSubKey(key.key) ? '#inpSubKeyCap' :
    '#inpKeyCap');
  ...
  if(key.fieldType == 'id' || key.key.data('id').startsWith('T_new_') || o.ctrl) {
    const idTarget = $(builder.dragDrop.isSubKey(key.key) ? '#inpSubKeyName' : '#inpKeyName');
    const chars = [...text]; // Split o.text into array of codepoints, keeping surrogate pairs together
    let id = chars.reduce((prev,curr) => prev+'_'+builder.dragDrop.charToUnicodeValue(curr).toUpperCase(), 'U');
    idTarget.val(id);
