...
function do_build() {
  ...
  cp_if_exists "$WIN32_TARGET_PATH/kmdbrowserhost.pdb" "$DEVELOPER_PROGRAM"
...
function do_install() {
  ...
  cp_if_exists "$DEVELOPER_PROGRAM/kmdbrowserhost.pdb" "$INSTALLPATH_KEYMANDEVELOPER/kmdbrowserhost.pdb"
}
