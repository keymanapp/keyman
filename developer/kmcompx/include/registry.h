...
protected:
	...
	KMX_BOOL WrapError(KMX_DWORD res);
...
public:
	...
	KMX_BOOL CloseKey(void);
	KMX_BOOL GetKeyNames(LPSTR AKey, int len, int n);
	KMX_BOOL GetValueNames(LPSTR AName, int len, int n);
	KMX_BOOL GetValueNames(LPWSTR AName, int len, int n);
	KMX_BOOL KeyExists(LPCSTR AKey);
	KMX_BOOL OpenKeyReadOnly(LPCSTR AKey);
	int ReadInteger(LPCSTR AName);
	KMX_BOOL ReadString(LPCSTR AName, LPSTR AValue, int len);
	KMX_BOOL ReadString(LPCWSTR AName, LPWSTR AValue, int len);
	KMX_BOOL ValueExists(LPCSTR AName);
	KMX_BOOL ValueExists(LPCWSTR AName);
};
...
protected:
	...
	KMX_BOOL IntRecursiveDeleteKey(HKEY hkey);

public:
	RegistryFullAccess(HKEY AhRootKey) : RegistryReadOnly(AhRootKey) {};
	~RegistryFullAccess() {};

	KMX_BOOL RecursiveDeleteKey(LPCSTR AKey);
	KMX_BOOL CreateKey(LPCSTR AKey);
	KMX_BOOL DeleteKey(LPCSTR AKey);
	KMX_BOOL DeleteValue(LPCSTR AName);
	KMX_BOOL OpenKey(LPCSTR AKey, KMX_BOOL ACreate);
	KMX_BOOL WriteInteger(LPCSTR AName, int AValue);
	KMX_BOOL WriteString(LPCSTR AName, LPSTR AValue);
	KMX_BOOL WriteString(LPCWSTR AName, LPWSTR AValue);
};
