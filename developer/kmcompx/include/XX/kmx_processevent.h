...
#include "../kmcompx.h"   // old: #include "kmx_base.h"   //_S
#include "../Compiler.h"  // #include "kmx_file.h"
...
private:
  ...
  KMX_INTKEYBOARDINFO m_keyboard = { 0 };
  KMX_DWORD m_modifiers = 0;

  // File loading //
  KMX_BOOL LoadKeyboard(km_kbp_path_name fileName, LPKMX_KEYBOARD *lpKeyboard);
...
#else
  LPKMX_KEYBOARD FixupKeyboard(PKMX_BYTE bufp, PKMX_BYTE base);
#endif

  KMX_BOOL ReleaseKeyboardMemory(LPKMX_KEYBOARD kbd);

  // Keystroke Processing //

  KMX_BOOL ProcessGroup(LPKMX_GROUP gp, KMX_BOOL *pOutputKeystroke);
  KMX_BOOL ContextMatch(LPKMX_KEY kkp);
  int PostString(PKMX_WCHAR str, LPKMX_KEYBOARD lpkb, PKMX_WCHAR endstr, KMX_BOOL *pOutputKeystroke);

  //Platform tests //

  KMX_BOOL IsMatchingBaseLayout(PKMX_WCHAR layoutName);
  KMX_BOOL IsMatchingPlatformString(PKMX_WCHAR platform);
  KMX_BOOL IsMatchingPlatform(LPKMX_STORE s);

  //Utility functions //

  void DeleteInternalDebugItems();
  void CreateInternalDebugItems();
  PKMX_WCHAR  GetSystemStore(LPKMX_KEYBOARD kb, KMX_DWORD SystemID);
...
public:
  ...
  LPKMX_INTKEYBOARDINFO GetKeyboard();
};
...
extern KMX_BOOL g_debug_ToConsole,  g_silent;
extern KMX_BOOL g_debug_KeymanLog;
