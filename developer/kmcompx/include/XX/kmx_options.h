...
// old #include <keyman/keyboardprocessor.h>
...
public:
  KMX_Options(LPKMX_INTKEYBOARDINFO kp, KMX_Actions& actions) : _kp(kp), m_actions(actions) {
  ...
  KMX_STORE const * begin() const;
  KMX_STORE const * end() const;
};
...
inline
KMX_STORE const * KMX_Options::begin() const {
  return _kp->Keyboard->dpStoreArray;
}

inline
KMX_STORE const * KMX_Options::end() const {
