...
KMX_BOOL KMX_ProcessEvent::ProcessEvent(
  ...
  LPKMX_KEYBOARD kbd = m_keyboard.Keyboard;
  ...
  LPKMX_GROUP gp = &kbd->dpGroupArray[kbd->StartGroup[BEGIN_UNICODE]];
...
/*
  ...
  if(*kkp->dpContext == UC_SENTINEL && *(kkp->dpContext+1) == CODE_NUL)
    u16cpy(m_miniContext,  m_context.Buf(xstrlen_ignoreifopt(kkp->dpContext)-1) );  // I3162   // I3536
  else
    u16cpy(m_miniContext,  m_context.Buf(xstrlen_ignoreifopt(kkp->dpContext)) );  // I3162   // I3536
...
KMX_BOOL KMX_ProcessEvent::IsMatchingPlatformString(PKMX_WCHAR platform)  // I3432
{
  ...
  u16cpy(t,  m_environment.platform().c_str());
