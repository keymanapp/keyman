	...
	<xsl:template match="/">
        ...
        <script src="/app/sentry.init.js"></script>
          ...
          <xsl:when test="/Keyman/not_keyman">
            <div><xsl:value-of select="$locale/String[@Id='S_FontHelper_WindowsKeyboard']"/></div>
          </xsl:when>
				  <xsl:when test="not(/Keyman/Fonts)">
				    <div><xsl:value-of select="$locale/String[@Id='S_FontHelper_NonUnicodeKeyboard']"/></div>
				  </xsl:when>
					<xsl:when test="count(/Keyman/Fonts/Font[@Coverage &gt; 90]) > 0">
						<div><xsl:value-of select="$locale/String[@Id='S_FontHelper_MatchedFonts']"/></div>
						<div>
							<table>
						...
						<xsl:if test="count(/Keyman/Fonts/Font[@Coverage &lt;= 90 and @Coverage &gt; 50]) &gt; 0">
							<div><xsl:value-of select="$locale/String[@Id='S_FontHelper_PossibleFonts']"/></div>
							<div>
								<table>
					...
					<xsl:when test="count(/Keyman/Fonts/Font[@Coverage &lt;= 90 and @Coverage &gt;= 50]) > 0">
						<div><xsl:value-of select="$locale/String[@Id='S_FontHelper_PossibleFontsOnly']"/></div>
						<div>
							<table>
								<xsl:apply-templates select="/Keyman/Fonts/Font[@Coverage &lt;= 90 and @Coverage &gt;= 50]" />
							</table>
					...
					<xsl:otherwise>
						<div><xsl:copy-of select="$locale/String[@Id='S_FontHelper_NoFonts'][1]"/></div>
					</xsl:otherwise>
				</xsl:choose>
