...
if (!window['keyman']['ui']['name']) {
    ...
    try {
        ...
        ui._ShowSelected = function () {
            var _rv, kbd = keymanweb['getActiveKeyboard'](), lgc = keymanweb['getActiveLanguage'](), kList = ui._KeymanWeb_KbdList.childNodes, _r = /^KMWSel_(.*)\$(.*)$/;
        ...
        ui._FocusFirstInput = function () {
            var i, ip = null, tp = null, iList = document.getElementsByTagName("input"), tList = document.getElementsByTagName("textarea");
        ...
        ui['initialize'] = ui.Initialize = function () {
            ...
            if (!keymanweb['initialized']) {
                window.setTimeout(ui.Initialize, 250);
                return;
            }
        ...
        ui.registerKeyboard = function (Lki, Lkl, Lkn, Lklc, Lkrc) {
            var _li2 = util['createElement']('li'), _a2 = util['createElement']('a'), _t = Lkn.replace(/\s?keyboard/i, '');
            if (Lkl) {
                var lg = Lkl.split(',')[0];
                if (Lkn.search(lg) == -1)
                    _t = lg + ' (' + _t + ')';
