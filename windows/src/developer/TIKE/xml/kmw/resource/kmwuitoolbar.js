...
if (!window['keyman']['ui']['name']) {
    ...
    try {
        ...
        ui['initialize'] = ui.initToolbarUI = function () {
            if (!keymanweb['initialized'] || ui.init)
                return;
        ...
        **/
        ui.addKeyboardsToMap = function () {
            ...
            for (var i in ui.regions) {
                ...
                for (var j = 0; j < Keyboards.length; j++) {
                    ...
                    var bcpSubtags = keymanweb['util']['getLanguageCodes'](Keyboards[j]['LanguageCode']);
        ...
        ui.findListedKeyboard = function (lang) {
            for (var i = 0; i < ui.listedKeyboards.length; i++)
                if (ui.listedKeyboards[i].lang.id == lang.id)
                    return i;
            return null;
        };
        /**
        ...
        ui.showKeyboardsForLanguage = function (event, lang) {
            ...
            if (e) {
                if (e.style.display == 'block')
                    return ui.hideKeyboardsForLanguage(event);
         ...
         * @return      {boolean}
         **/
        ui.selectKeyboard = function (event, lang, kbd, updateKeyman) {
        ...
        ui.enableControls = function () {
            ...
            if (keymanweb['isCJK'](ui.selectedKeyboard))
                hideOskButton = true;
        ...
        ui.focusControlEvent = function (params) {
            ...
            if (t.tagName.toLowerCase() == 'textarea' || (t.tagName.toLowerCase() == 'input' && t.type.toLowerCase() == 'text')) {
                ...
                if (ui.pluck(t, 'kmw_disable')) {
                    ...
                    if (ui.toolbarNode.className != '')
                        ui.toolbarNode.className = '';
                    var offsetX, offsetY;
        ...
        ui.blurControlEvent = function (params) {
            ...
            if (ui.oskButtonNode.style.display != 'none')
                ui.oskButtonNode.className = 'kmw_button_disabled';
            if (!params.event)
        ...
        ui.changeKeyboardEvent = function (p) {
            ui.lastSelectedKeyboard = null;
            var kbName = p['internalName'], lgName = keymanweb['util']['getLanguageCodes'](p['languageCode'])[0];
            if (lgName != '' && kbName != '') {
                ...
                ui.lastSelectedKeyboard = __assign({}, p);
            }
            return true;
        };
        ...
        ui.onShowOSK = function (oskPosition) {
            if (ui.init)
                ui.oskButtonNode.className = 'kmw_button_selected';
        ...
        ui.onHideOSK = function (p) {
            if (ui.init && p['HiddenByUser'])
                ui.oskButtonNode.className = 'kmw_button';
        };
        ...
        ui.showKeyboardsPopup = function (event) {
            ...
            if (ui.toolbarNode.className == 'kmw_controls_disabled')
                return ui.eventCapture(event);
            ui.hideKeyboardsForLanguage(null);
            if (ui.selectorNode.className == 'kmw_over')
                return ui.hideKeyboardsPopup(event);
        ...
        **/
        ui.hideAllPopups = function (event) {
            var e = ui.keyboardsForLangPopup;
            if ((!e || (e.style.display == 'none')) && (ui.selectorNode.className == ''))
                return true;
            ui.hideKeyboardsPopup(event);
        ...
        ui.PopupDismissal = function (event) {
            ...
            if (t.nodeName == '#document')
                ui.hideAllPopups(event); //KMEW-41, fixed for build 356.
        ...
        ui.loadCookie = function () {
            ...
            if (c['current'] != undefined)
                currentKeyboard = c['current'].split(':')[0];
            c = util['loadCookie']("KeymanWeb_Toolbar");
            if (c['region'] != undefined)
                ui.selectedRegion = c['region'];
            ...
            // initialize to the current (default) language and keyboard, if set by KMW
                ...
                var kbName = keymanweb['getActiveKeyboard'](), lgName = keymanweb['getActiveLanguage']();
                if (lgName != '' && kbName != '') {
                    var lg = ui.languages[lgName];
                    if (lg != null) {
                        for (var j = 0; j < lg.keyboards.length; j++) {
