    ...
    <div class="page" id="Keyboards">
      <div id="uppertext1" style="clear: all; display: block;" >
        ...
        <div class="pagetext">
          ...
          <p>Click the New Keyboard button to create a keyboard for any device.  This will open the Keyboard Editor, where you can 
          create a keyboard visually, or programatically with the Keyman Keyboard Language.</p>
  ...
  <xsl:template mode="filedetails" match="/KeymanDeveloperProject/Files/File[FileType='.kmn']">
    ...
    <table class="filedetailtext">
      ...
      <xsl:if test="FileVersion">
        <tr>
          <th>Version:</th><td><xsl:value-of select="FileVersion"/></td>
        </tr>
      </xsl:if>
      <xsl:if test="Details/Message">
        <tr>
          <th>Message:</th><td><xsl:value-of select="Details/Message"/></td>
        </tr>
      </xsl:if>
    </table>
  </xsl:template>
