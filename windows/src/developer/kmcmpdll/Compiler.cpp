...
KMX_BOOL IsSameToken(PWSTR *p, WCHAR const * token);
KMX_DWORD GetRHS(PFILE_KEYBOARD fk, PWSTR p, PWSTR buf, int bufsize, int offset, int IsUnicode);
PWSTR GetDelimitedString(PWSTR *p, WCHAR const * Delimiters, KMX_WORD Flags);
KMX_DWORD GetXString(PFILE_KEYBOARD fk, PWSTR str, WCHAR const * token, PWSTR output, int max, int offset, PWSTR *newp, int isVKey,
  int isUnicode);
...
KMX_DWORD ProcessStoreLine(PFILE_KEYBOARD fk, PWSTR p);
KMX_DWORD AddDebugStore(PFILE_KEYBOARD fk, WCHAR const * str);
KMX_DWORD ProcessKeyLine(PFILE_KEYBOARD fk, PWSTR str, KMX_BOOL IsUnicode);
KMX_DWORD ProcessEthnologueStore(PWSTR p); // I2646
KMX_DWORD ProcessHotKey(PWSTR p, DWORD *hk);
KMX_DWORD ImportBitmapFile(PFILE_KEYBOARD fk, PWSTR szName, PDWORD FileSize, PBYTE *Buf);

KMX_DWORD ExpandKp(PFILE_KEYBOARD fk, PFILE_KEY kpp, KMX_DWORD storeIndex);

KMX_DWORD ReadLine(HANDLE hInfile, PWSTR str, KMX_BOOL PreProcess);
...
KMX_DWORD process_if(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);
KMX_DWORD process_reset(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);
KMX_DWORD process_set(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);
KMX_DWORD process_save(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);
KMX_DWORD process_platform(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);  // I3430
KMX_DWORD process_baselayout(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);  // I3430
KMX_DWORD process_set_synonym(KMX_DWORD dwSystemID, PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);  // I3437
KMX_DWORD process_expansion(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx, int max);

KMX_BOOL IsValidKeyboardVersion(WCHAR *dpString);   // I4140
...
//KMX_BOOL KMX_IsRelativePath(KMX_CHAR *p)                                            // S: ready
//int KMX_CheckUTF16(int n)                                                           // S: ready
//int KMX_UTF32ToUTF16(int n, int *n1, int *n2)                                       // S: ready
...
KMX_DWORD ProcessBeginLine(PFILE_KEYBOARD fk, PWSTR p)
{
  WCHAR tstr[128];
  PWSTR q, pp;
  int BeginMode;
...
KMX_DWORD ValidateMatchNomatchOutput(PWSTR p) {
  while (p && *p) {
    if (*p == UC_SENTINEL) {
      switch (*(p + 1)) {
      case CODE_CONTEXT:
...
KMX_DWORD AddStore(PFILE_KEYBOARD fk, KMX_DWORD SystemID, WCHAR const * str, DWORD *dwStoreID)
{
  PFILE_STORE sp;
...
KMX_DWORD CheckStatementOffsets(PFILE_KEYBOARD fk, PFILE_GROUP gp, PWSTR context, PWSTR output, PWSTR key) {
  PWSTR p, q;
  int i;
 ...
 */
KMX_BOOL CheckContextStatementPositions(PWSTR context) {
 ...
 */
KMX_DWORD CheckUseStatementsInOutput(PWSTR output) {   // I4867
...
KMX_BOOL strvalidchrs(PWSTR q, WCHAR const * chrs)
{
  for (; *q; q++)
...
KMX_DWORD GetXString(PFILE_KEYBOARD fk, PWSTR str, WCHAR const * token, PWSTR output, int max, int offset, PWSTR *newp, int isVKey, int isUnicode)
{
  KMX_DWORD err;
  PWSTR p = str, q, r;
...
KMX_DWORD process_if_synonym(KMX_DWORD dwSystemID, PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);  // I3430
...
KMX_DWORD process_if(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)  // I3431
{
...
KMX_DWORD ReadLine(HANDLE hInfile, PWSTR wstr, KMX_BOOL PreProcess)
{
  DWORD len;
  PWSTR p;
...
PKMX_STR KMX_wstrtostr(PWSTR in)
{
  PKMX_STR result;
  size_t len;
...
KMX_BOOL KMX_AddCompileMessage(DWORD msg)
{
  KMX_CHAR szText[SZMAX_ERRORTEXT + 1 + 280];
...
KMX_DWORD KMX_ValidateMatchNomatchOutput(PKMX_WSTR p) {
  while (p && *p) {
    if (*p == UC_SENTINEL) {
      switch (*(p + 1)) {
      case CODE_CONTEXT:
...
KMX_DWORD KMX_ParseLine(PFILE_KEYBOARD fk, PWSTR str)
{
  PWSTR p, q, pp;
  PFILE_GROUP gp;
  KMX_DWORD msg;
...
KMX_DWORD KMX_AddStore(PFILE_KEYBOARD fk, KMX_DWORD SystemID, WCHAR const * str, DWORD *dwStoreID)
{
  PFILE_STORE sp;
...
KMX_BOOL KMX_IsValidKeyboardVersion(WCHAR *dpString) {   // I4140
  /* version format \d+(\.\d+)*  e.g. 9.0.3, 1.0, 1.2.3.4, 6.2.1.4.6.4, blank is not allowed */
...
KMX_DWORD KMX_CheckStatementOffsets(PFILE_KEYBOARD fk, PFILE_GROUP gp, PWSTR context, PWSTR output, PWSTR key) {
  PWSTR p, q;
  int i;
  for (p = output; *p; p = incxstr(p)) {
    if (*p == UC_SENTINEL) {
...
KMX_DWORD KMX_ProcessKeyLine(PFILE_KEYBOARD fk, PWSTR str, KMX_BOOL IsUnicode)
{
  PWSTR p, pp;
  KMX_DWORD msg;
  PFILE_GROUP gp;
...
PWSTR KMX_GetDelimitedString(PWSTR *p, WCHAR const * Delimiters, KMX_WORD Flags)
{
  PWSTR q, r;
  WCHAR dOpen, dClose;
...
KMX_DWORD KMX_GetXString(PFILE_KEYBOARD fk, PWSTR str, WCHAR const * token, PWSTR output, int max, int offset, PWSTR *newp, int isVKey, int isUnicode)
{
  KMX_DWORD err;
  PWSTR p = str, q, r;
  int type, mx = 0, n, n1, n2, tokenFound = FALSE, z, sFlag = 0, j;
...
KMX_DWORD KMX_process_if_synonym(KMX_DWORD dwSystemID, PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx);  // I3430

KMX_DWORD KMX_process_baselayout(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)  // I3430
{
  /* baselayout(<XString+outs>) */
  return process_if_synonym(TSS_BASELAYOUT, fk, q, tstr, mx);
}

KMX_DWORD KMX_process_platform(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)  // I3430
{
  /* platform(<XString+outs>) */
  return process_if_synonym(TSS_PLATFORM, fk, q, tstr, mx);
}
...
KMX_DWORD KMX_process_if(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)  // I3431
{
  /* if(<store> <'='|'!='> <XString+outs>) */
  KMX_DWORD i, code; KMX_DWORD nnot = FALSE;
  LPWSTR r = q, s = q;
...
KMX_DWORD KMX_process_reset(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)
{
  /* reset(<store>) */
  KMX_DWORD i;
  for (i = 0; i < fk->cxStoreArray; i++)
...
KMX_DWORD KMX_process_expansion(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx, int max) {
  KMX_BOOL isVKey = FALSE;
...
KMX_DWORD KMX_process_set_synonym(KMX_DWORD dwSystemID, PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)  // I3437
{
  /* set(<store> <'='> <XString+outs>), layer */
  KMX_DWORD code = CODE_SETSYSTEMSTORE;
  PWCHAR temp = new WCHAR[GLOBAL_BUFSIZE], r;
...
KMX_DWORD KMX_process_set(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)
{
  /* set(<store> <'='> <XString+outs> */
  LPWSTR r = q, s = q;  // I3440
  while (*s && *s != L' ' && *s != L'=') s++;
...
KMX_DWORD KMX_process_save(PFILE_KEYBOARD fk, LPWSTR q, LPWSTR tstr, int *mx)
{
  /* save(<store>) */
  KMX_DWORD i;
  for (i = 0; i < fk->cxStoreArray; i++)
...
int KMX_GetGroupNum(PFILE_KEYBOARD fk, PWSTR p)
{
  PFILE_GROUP gp;
  KMX_DWORD i;
...
KMX_DWORD KMX_ProcessEthnologueStore(PWSTR p) // I2646
{
  KMX_DWORD res = CERR_None;
  PWSTR q = NULL;
  while (*p)
...
KMX_DWORD KMX_ProcessHotKey(PWSTR p, DWORD *hk)
{
  PWSTR q, r;
  KMX_DWORD sFlag;
  int j, i;
...
void KMX_safe_wcsncpy(PWSTR out, PWSTR in, int cbMax)
{
  wcsncpy_s(out, cbMax, in, cbMax - 1);  // I3481
  out[cbMax - 1] = 0;
}

KMX_BOOL KMX_IsSameToken(PWSTR *p, WCHAR const * token)
{
  PWSTR q;
  q = *p;
  while (iswspace(*q)) q++;
