/*
  ...
  History:          20 Jun 2006 - mcdurdin - Initial version
                    ...
                    25 Oct 2016 - mcdurdin - I5135 - Remove product and licensing references from Developer projects
                       ...
                       TODO - S - adress LNK 2019, LNK 1120
...
KMX_DWORD GetXString(PFILE_KEYBOARD KMX_fk, PKMX_WCHAR str, KMX_WCHAR const * token, PKMX_WCHAR output, int max, int offset, PKMX_WCHAR *newp, int isVKey, int isUnicode);
...
NamedCodeConstants *CodeConstants = NULL;  //  _S2  NamedCodeConstants_S2 *CodeConstants_S2 = NULL;
/* Compile target */
...
#define DebugLog(msg,...) (km::kbp::kmx::ShouldDebug() ? km::kbp::kmx::DebugLog1(__FILE__, __LINE__, __FUNCTION__, (msg),__VA_ARGS__) : 0)
...
KMX_BOOL AddCompileMessage(KMX_DWORD msg)
{
  ...
  if (msg & CERR_MEMORY)
  {
    ...
    memcpy(szText,  "Fatal Error: Out of Memory", sizeof(szText)); // _S2 lstrcpyA((LPSTR) (szText), "Fatal Error: Out of Memory"); //lstrcpy(szText, "Fatal Error: Out of Memory");
  ...
  if (ErrChr > 0)
    ...
    sprintf(strchr(szText, 0), " chr:%d", ErrChr);            // _S2 wsprintf(strchr(szText, 0), " chr:%d", ErrChr);
  if (*ErrExtra)
...
if (FSaveDebug)
  ...
  AddDebugStore(fk, BeginMode == BEGIN_UNICODE ? KMX_DEBUGSTORE_BEGIN u"Unicode" : KMX_DEBUGSTORE_BEGIN u"ANSI");    //KMX_AddDebugStore(fk, BeginMode == BEGIN_UNICODE ? DEBUGSTORE_BEGIN L"Unicode" : DEBUGSTORE_BEGIN L"ANSI");
return CERR_None;
}
  ...
  case T_MATCH:
    if (fk->currentGroup == 0xFFFFFFFF) return CERR_CodeInvalidInThisSection;
    {
      ...
      if (FSaveDebug)
      {
        ...
        u16printf(&p_tstr, 'd',  0x0020 , createIntVector((int) fk->currentGroup), (PKMX_WCHAR) KMX_DEBUGSTORE_MATCH, gp->szName);  // I3481   //swprintf(tstr, _countof(tstr), L"%ls%d %ls", DEBUGSTORE_MATCH, (int) fk->currentGroup, gp->szName);  // I3481
  ...
  case T_NOMATCH:
    if (fk->currentGroup == 0xFFFFFFFF) return CERR_CodeInvalidInThisSection;
    {
      ...
      if (FSaveDebug)
      {
        ...
        PKMX_WCHAR p_tstr;    // _S2        
        /* Record a system store for the line number of the begin statement */
        u16printf(&p_tstr,'d',  0x0020 , createIntVector(  fk->currentGroup),(PKMX_WCHAR) KMX_DEBUGSTORE_NOMATCH, gp->szName);  // I3481     //swprintf(tstr, _countof(tstr), L"%ls%d %ls", DEBUGSTORE_NOMATCH, fk->currentGroup, gp->szName);  // I3481
...
KMX_DWORD ProcessGroupLine(PFILE_KEYBOARD fk, PKMX_WCHAR p)
{
  ...
  gp->fUsingKeys = FALSE;
  if (IsSameToken(&p, u"using") && IsSameToken(&p, u"keys")) gp->fUsingKeys = TRUE;
  ...
  if (FSaveDebug)
  {
    ...
    PKMX_WCHAR p_tstr;    // _S2     
    /* Record a system store for the line number of the begin statement */        
    u16printf(&p_tstr,'d',  0x0020 , createIntVector(  fk->cxGroupArray - 1),(PKMX_WCHAR) KMX_DEBUGSTORE_GROUP, gp->szName);  // I3481     //swprintf(tstr, _countof(tstr), u"%ls%d %ls", DEBUGSTORE_GROUP, fk->cxGroupArray - 1, gp->szName);  // I3481
    AddDebugStore(fk, tstr);
  }
...
PKMX_WCHAR GetDelimitedString(PKMX_WCHAR *p, KMX_WCHAR const * Delimiters, KMX_WORD Flags)
{
  ...
  *p = r;								    // Update pointer position
  ...
  return (PKMX_WCHAR) q;							// Return delimited string
}
    ...
    do
    {
      ...
      case 0:                                            // _S2  if (_wcsnicmp(p, L"deadkey", z = 7) == 0 ||
          ...
          u16ncmp(p, u"dk", z = 2) == 0)
        {
          ...
          tstr[mx++] = GetDeadKey(KMX_fk, q); //atoiW(q); 7-5-01: named deadkeys
          tstr[mx] = 0;
        }
        else
        {
      ...
      case 4:
        ...
        {
          VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_90, CERR_90FeatureOnly_IfSystemStores);
          if (sFlag) return CERR_InvalidInVirtualKeySection;
          p += 10;
          q = GetDelimitedString(&p, u"()", GDS_CUTLEAD | GDS_CUTFOLL);
          if (!q || !*q) return CERR_InvalidToken;
          err = process_baselayout(KMX_fk, q, tstr, &mx);
      ...
      case 5:
        ...
        {
          VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_80, CERR_80FeatureOnly);
          if (sFlag) return CERR_InvalidInVirtualKeySection;
          p += 2;
          q = GetDelimitedString(&p, u"()", GDS_CUTLEAD | GDS_CUTFOLL);
          if (!q || !*q) return CERR_InvalidIf;

          err = process_if(KMX_fk, q, tstr, &mx);
          if (err != CERR_None) return err;
        }
        else
        {
...

          {
            ...
            for (i = 0; i < KMX_fk->cxStoreArray; i++)
            {
              if (u16icmp(r, KMX_fk->dpStoreArray[i].szName) == 0) break;
            }
            if (i == KMX_fk->cxStoreArray) return CERR_StoreDoesNotExist;
      ...
      case 7:
        ...
        if (u16ncmp(p, u"context", 7) == 0)
        {
          ...
          if (q && *q)
          {
            VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_60, CERR_60FeatureOnly_Contextn);
            int n1;
            n1 = atoiW(q);
        ...
        {
          VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_501, CERR_501FeatureOnly_Call);
          ...
          if (!IsValidCallStore(&KMX_fk->dpStoreArray[i])) return CERR_InvalidCall;
          CheckStoreUsage(KMX_fk, i, FALSE, FALSE, TRUE);
      ...
      case 8:
        ...
        {
          VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_70, CERR_70FeatureOnly)
            if (sFlag) return CERR_AnyInVirtualKeySection;
          p += 6;
          q = GetDelimitedString(&p, u"()", GDS_CUTLEAD | GDS_CUTFOLL);
          if (!q || !*q) return CERR_InvalidAny;
      ...
      case 10:
        if (u16ncmp(p, u"reset", 5) == 0)
        {
          ...
          err = process_reset(KMX_fk, q, tstr, &mx);
          if (err != CERR_None) return err;
        }
        else
        {
      ...
      case 11:
        ...
        if (sFlag & (LCTRLFLAG | LALTFLAG | RCTRLFLAG | RALTFLAG | CAPITALFLAG | NOTCAPITALFLAG | NUMLOCKFLAG | NOTNUMLOCKFLAG | SCROLLFLAG | NOTSCROLLFLAG) &&
          ...
          VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_100, 0);
        }
        //printf("sFlag: %x\n", sFlag);
        ...
        while (*q != ']')
        {
          if (*q == '\'' || *q == '"')
          {
            VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_60, CERR_60FeatureOnly_VirtualCharKey);
          ...
          if (i == VK__MAX + 1)
          {
            VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_90, CERR_InvalidToken);

            i = GetVKCode(KMX_fk, vkname);  // I3438
            if (i == 0)
              return CERR_InvalidToken;
          }
      ...
      case 14:
        if (u16ncmp(p, u"set", 3) == 0)
        {
          ...
          err = process_set(KMX_fk, q, tstr, &mx);
          if (err != CERR_None) return err;
        }
        else if (u16ncmp(p, u"save", 4) == 0)
        {
          ...
          err = process_save(KMX_fk, q, tstr, &mx);
          if (err != CERR_None) return err;
        }
        else
        {
      ...
      case 15:
        ...
        continue;
      case 16:
        VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_60, CERR_60FeatureOnly_NamedCodes);
        q = p + 1;
        ...
        *q = c;
        ...
        if (i < 0xFFFFFFFFL) CheckStoreUsage(KMX_fk, i, TRUE, FALSE, FALSE);   // I2993
        if (n > 0xFFFF)
        {
      ...
      case 17:
        ...
        q = GetDelimitedString(&p, u"()", GDS_CUTLEAD | GDS_CUTFOLL);
        if (!q || !*q) return CERR_InvalidToken;
        err = process_platform(KMX_fk, q, tstr, &mx);
        if (err != CERR_None) return err;
        continue;
      case 18:  // I3437
        if (u16ncmp(p, u"layer", 5) != 0) return CERR_InvalidToken;
        VERIFY_KEYBOARD_VERSION(KMX_fk, VERSION_90, CERR_90FeatureOnly_SetSystemStores);
        if (sFlag) return CERR_InvalidInVirtualKeySection;
        p += 5;
        q = GetDelimitedString(&p, u"()", GDS_CUTLEAD | GDS_CUTFOLL);
        if (!q || !*q) return CERR_InvalidToken;
        err = process_set_synonym(TSS_LAYER, KMX_fk, q, tstr, &mx);
        if (err != CERR_None) return err;
        continue;
      case 19:  // #2241
        ...
        if (sFlag) return CERR_InvalidInVirtualKeySection;
        p += 2;
        err = process_expansion(KMX_fk, p, tstr, &mx, max);
        if (err != CERR_None) return err;
        continue;
...
KMX_DWORD process_if(PFILE_KEYBOARD fk, LPKMX_WCHAR q, LPKMX_WCHAR tstr, int *mx)  // I3431
{
  ...
  *r = 0;
  ...
  if (r[0] == '&')
  {
    VERIFY_KEYBOARD_VERSION((PFILE_KEYBOARD) fk, VERSION_90, CERR_90FeatureOnly_IfSystemStores);    
    for (i = 0; StoreTokens[i]; i++)
    {
...
KMX_DWORD process_save(PFILE_KEYBOARD fk, LPKMX_WCHAR q, LPKMX_WCHAR tstr, int *mx)
{
  ...
  for (i = 0; i < fk->cxStoreArray; i++)
  {
    if (u16icmp((PKMX_WCHAR) q, fk->dpStoreArray[i].szName) == 0) break;
  }
  if (i == fk->cxStoreArray) return CERR_StoreDoesNotExist;
  CheckStoreUsage(fk, i, FALSE, TRUE, FALSE);
...
void RecordDeadkeyNames(PFILE_KEYBOARD fk)
{
  ...
  for (i = 0; i < fk->cxDeadKeyArray; i++)
  {
    u16printf(&p_buf,'d', 0x0020, createIntVector( (int)i), (PKMX_WCHAR) KMX_DEBUGSTORE_DEADKEY, fk->dpDeadKeyArray[i].szName) ;      // _S2 swprintf(buf, _countof(buf), L"%ls%d %ls", DEBUGSTORE_DEADKEY, (int)i, fk->dpDeadKeyArray[i].szName);  // I3481
