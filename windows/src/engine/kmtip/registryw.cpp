...
{
	RegistryReadOnlyW *reg = new RegistryReadOnlyW(HKEY_LOCAL_MACHINE);
	if(!reg->OpenKeyReadOnly(REGSZ_KeymanInstalledKeyboardsLMW) || !reg->OpenKeyReadOnly(kbname))
	{
		delete reg;
...
{
	RegistryReadOnlyW *reg = new RegistryReadOnlyW(HKEY_CURRENT_USER);
	if(!reg->OpenKeyReadOnly(REGSZ_KeymanActiveKeyboardsW) || !reg->OpenKeyReadOnly(kbname))
	{
		delete reg;
		return NULL;
	}
