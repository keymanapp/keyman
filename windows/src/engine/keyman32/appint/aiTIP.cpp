...
BOOL TIPProcessKeyInternal(
  ...
  if ((_td->LastKey == wParam && _td->LastScanCode == SCAN_FLAG_KEYMAN_KEY_EVENT && _td->LastTransition == keyTransition)
    ...
    SendDebugMessageFormat("Virtual Key was generated by Keyman [Scan=%x wParam=%x] [LastKey=%x LastScanCode=%x LastTransition=%x]",
      scan, wParam, _td->LastKey, _td->LastScanCode, _td->LastTransition);
