...
void UpdateLastKeyCache(PKEYMAN64THREADDATA _td, WPARAM wParam, BYTE scan, BYTE keyTransition) {
  ...
  _td->LastTransition = keyTransition;
}
...
BOOL TIPProcessKeyInternal(
  ...
  UpdateLastKeyCache(_td, wParam, scan, keyTransition);
  SendDebugEntry();
  ...
  // If this key event was generated by Keyman, then we should return FALSE without
  // processing it in the core processor. If it is a CapsLock key we update our
  // Globals::ShiftState. Since this could run before kmhook_getmessage processes the event,
  // we also check for the scan code flag that we set when generating a key event from Keyman.
  if ((prevKey == wParam && prevScanCode == SCAN_FLAG_KEYMAN_KEY_EVENT && prevKeyTransition == keyTransition)
    ...
    SendDebugMessageFormat("Virtual Key was generated by Keyman [Scan=%x wParam=%x] [prevKey=%x prevScanCode=%x prevKeyTransition=%x]",
      scan, wParam, prevKey, prevScanCode, prevKeyTransition);
