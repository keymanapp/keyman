...
TEST(KEYBOARDOPTIONS, SetupCoreEnvironment) {
  ...
  EXPECT_EQ(km_core_keyboard_load(dummyPath, &kp->lpCoreKeyboard), KM_KBP_STATUS_OK);
  EXPECT_EQ(km_core_state_create(kp->lpCoreKeyboard, core_env_opts, &kp->lpCoreKeyboardState), KM_KBP_STATUS_OK);
  ...
  for (int i = 0; i < 4; ++i) {
    EXPECT_EQ(km_core_state_option_lookup(kp->lpCoreKeyboardState, KM_KBP_OPT_ENVIRONMENT, expected_items[i].key, &retValue), KM_KBP_STATUS_OK);
    value = retValue;
  ...
  EXPECT_EQ(km_core_state_options_update(kp->lpCoreKeyboardState, expected_items), KM_KBP_STATUS_OK);

  for (int i = 0; i < 4; ++i) {
    EXPECT_EQ(
        km_core_state_option_lookup(kp->lpCoreKeyboardState, KM_KBP_OPT_ENVIRONMENT, expected_items[i].key, &retValue),
        KM_KBP_STATUS_OK);
    value         = retValue;
    expectedValue = expected_items[i].value;
    EXPECT_TRUE(expectedValue == value);
