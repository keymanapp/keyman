...
TEST_F(KMPROCESSACTIONS, processUnicodeChartest) {
  ...
  km_core_action_item itemAddChar = { KM_KBP_IT_CHAR, {0,}, {'A'}};
  ...
  km_core_action_item itemAddChar2 = {KM_KBP_IT_CHAR, {0,}, {testSurrogateChar}};
...
TEST_F(KMPROCESSACTIONS, processMarkertest) {
  ...
  uintptr_t marker               = 2;
  km_core_action_item itemAddMarker = {KM_KBP_IT_MARKER, {0,}, {marker}};
...
TEST_F(KMPROCESSACTIONS, processBackDeadkeytest) {
  ...
  km_core_action_item itemAddMarker = {KM_KBP_IT_MARKER, {0,}, {marker}};
  processMarker(&testApp, &itemAddMarker);
  km_core_action_item itemBackSpace = {KM_KBP_IT_BACK};
...
TEST_F(KMPROCESSACTIONS, processBackSurrogateTSFtest) {
  ...
  km_core_usv testSurrogateChar    = Uni_SurrogateToUTF32(0xD801, 0xDC37);  //𐐷';
  km_core_action_item itemAddChar  = {KM_KBP_IT_CHAR, {0,}, {testSurrogateChar}};
  processUnicodeChar(&testApp, &itemAddChar);
...
TEST_F(KMPROCESSACTIONS, processInvalidateContextTest) {
  ...
  km_core_option_item test_env_opts[] = {{u"hello", u"world", KM_KBP_OPT_KEYBOARD}, KM_KBP_OPTIONS_END};
  km_core_keyboard *testKB = nullptr;
  km_core_state *testState = nullptr;
  km_core_path_name dummyPath      = L"dummyActions.mock";
  EXPECT_EQ(km_core_keyboard_load(dummyPath, &testKB), KM_KBP_STATUS_OK);
  EXPECT_EQ(km_core_state_create(testKB, test_env_opts, &testState), KM_KBP_STATUS_OK);
