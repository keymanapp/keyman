...
static BOOL LoadKeyboardFile(LPSTR fileName, LPKEYBOARD *lpKeyboard, int *ProductIndex);
BOOL VerifyKeyboard(LPBYTE filebase, DWORD sz);

LPKEYBOARD FixupKeyboard(PBYTE bufp, PBYTE base, DWORD dwFileSize);

void Err(wchar_t *s) {
	wprintf(L"LoadKeyboard: %s, last error = %d\n", s, GetLastError());
}
...
/*	if(*LPDWORD(filebase) != FILEID_COMPILED)
	{
		if(!DecryptKeyboard(filebase, &sz, &kbp, ProductIndex)) { delete buf; Err("errDecrypt"); return FALSE; }
    delete buf;
#ifdef _WIN64
    buf = new BYTE[sz * 3];
    filebase = buf + sz*2;
    memcpy(filebase, kbp, sz);
    delete kbp;
#else
		filebase = buf = (LPBYTE) kbp;
#endif
	} */
  ...
  if(!kbp) {
	...
	/*if(!kbp->IsRegistered) {
		ErrSendDebugMessageFormat(0, sdmLoad, 0, "LoadKeyboard: Keyboard %s is not registered", fileName);
	}*/
