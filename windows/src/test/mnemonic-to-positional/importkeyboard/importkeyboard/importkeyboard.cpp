// importkeyboard.cpp : Defines the entry point for the console application.
//
...
public:
  ...
  int DeadKeyMap(int index, std::vector<DeadKey *> *deadkeys, int deadkeyBase) {
  ...
  bool LayoutRow(int MaxShiftState, LPKEY key, std::vector<DeadKey*> *deadkeys, int deadkeyBase) {
    ...
    for (int ss = 0; ss <= MaxShiftState; ss++) {
      ...
      for (int caps = 0; caps <= 1; caps++) {
        ...
        } else if ((caps == 1) && st.compare(this->GetShiftState((ShiftState) ss, (caps == 0))) == 0) {
...
int wmain(int argc, WCHAR* argv[]) {
  if(argc == 1) {
    puts("Usage: importkeyboard /list | [/kmw] hkl [output.kmn] ");
    puts(" /list shows a list of keyboards available on your system.\n");
    puts(" hkl should be an 8 hex digit Keyboard ID.  See /list to enumerate these.");
    puts(" /kmw should be specified to target KeymanWeb.  When using KeymanWeb, the imported keyboard will:");
    puts("    * Ignore CAPS/NCAPS");
    puts("    * Convert RALT to CTRL+ALT\n");
    puts(" If output.kmn is not specified, the filename of the source keyboard will be used; e.g. 00000409 will produce kbdus.kmn\n");
    return 1;
  }
  ...
  wsprintf(inputHKL, L"%08.8x", wcstol(argv[1], NULL, 16));
  ...
  HKEY hkeyBase, hkey;
  if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Control\\Keyboard Layouts", 0, KEY_READ, &hkeyBase) != ERROR_SUCCESS) {
    printf("Could not find %x in registry", inputHKL);
    return 2;
  }
  if(RegOpenKeyEx(hkeyBase, inputHKL, 0, KEY_READ, &hkey) != ERROR_SUCCESS) {
  ...
  for(UINT i = 0; i < kp->cxGroupArray; i++, gp++) {
    if(gp->fUsingKeys && gp->dpNoMatch == NULL) {
