...
function mockedSetup(queryResultsFile) {
  ...
  const keyboardRequisitioner = new KeyboardRequisitioner(new NodeKeyboardLoader(), mockedRequester, pathConfig);
...
describe("KeyboardRequisitioner", () => {
  ...
  describe('addLanguageKeyboards', function() {
    ...
    // A double-mocked query that allows language-list, then keyboard-stub query and returns
    // the actual objects WOULD be "nice to have", though getting as far as the previous
    // test strongly suggests that the functionality will integrate properly.
    //
    // Would be nice to verify:
    // - one keyboard stub per requested language
    // - each stub is registered in the cache.
  });
});