...
describe('ContextToken', function() {
  ...
  describe("<constructor>", () => {
    it("(model: LexicalModel)", async () => {
      ...
      assert.isEmpty(token.searchSpace.inputSequences);
    ...
    it("(model: LexicalModel, text: string)", () => {
      ...
      assert.isNotEmpty(token.searchSpace.inputSequences?.[0]);

      assert.equal(token.searchSpace.inputSequences[0].map((entry) => entry[0].sample.insert).join(''), 'and');
      token.searchSpace.inputSequences[0].forEach((entry) => assert.equal(entry[0].sample.deleteLeft, 0));
      assert.deepEqual(token.searchSpace.inputSequences[0], [..."and"].map((char) => {
        return [{
          sample: {
            insert: char,
            deleteLeft: 0
    ...
    it("(token: ContextToken", () => {
      ...
      assert.notEqual(clonedToken.searchSpace.inputSequences[0], baseToken.searchSpace.inputSequences[0]);
      assert.deepEqual(clonedToken.searchSpace.inputSequences[0], baseToken.searchSpace.inputSequences[0]);
  ...
  //     assert.sameOrderedMembers(merged.searchSpace.inputSequences[0].slice(0, 3), token1.searchSpace.inputSequences[0].slice());
  //     assert.sameOrderedMembers(merged.searchSpace.inputSequences[0].slice(3, 4), token2.searchSpace.inputSequences[0].slice());
  //     assert.sameOrderedMembers(merged.searchSpace.inputSequences[0].slice(4), token3.searchSpace.inputSequences[0].slice());
  //   });
