...
import { correction, generateSpaceSeed, getBestMatches, models, SearchCluster, SearchPath } from '@keymanapp/lm-worker/test-index';
...
describe('Correction Searching', () => {
  ...
  describe('with divergent SearchSpaces', () => {
    const buildPathFixture = () => {
      ...
      const tPath = new SearchPath(rootPath, distrib_t1, distrib_t1[0]);
      ...
      const thPath = new SearchPath(tPath, distrib_h2, distrib_h2[0]);
      const toPath = new SearchPath(tPath, distrib_o2, thPath.inputSource);
      const trPath = new SearchPath(tPath, distrib_i2, thPath.inputSource);

      const twoCharCluster = new SearchCluster([thPath, toPath, trPath]);
      ...
      const thvPath = new SearchPath(thPath, distrib_v3, distrib_v3[0]);
      const tovPath = new SearchPath(toPath, distrib_v3, thvPath.inputSource);
      const trvPath = new SearchPath(trPath, distrib_v3, thvPath.inputSource);
      ...
      const thvnPath = new SearchPath(thvPath, distrib_n4, distrib_n4[0]);
      const tovnPath = new SearchPath(tovPath, distrib_n4, thvnPath.inputSource);
      const trvnPath = new SearchPath(trvPath, distrib_n4, thvnPath.inputSource);
      const clvnPath = new SearchPath(clvPath, distrib_n4, thvnPath.inputSource);

      const thvrPath = new SearchPath(thvPath, distrib_v3r, {...thvnPath.inputSource, subsetId: generateSpaceSeed()});
      const tovrPath = new SearchPath(tovPath, distrib_v3r, thvrPath.inputSource);
      const trvrPath = new SearchPath(trvPath, distrib_v3r, thvrPath.inputSource);
      const clvrPath = new SearchPath(clvPath, distrib_v3r, thvrPath.inputSource);
    ...
    it('returns the same results, in order, from SearchCluster as from constituent SearchPaths', async () => {
      ...
      for(let i=0; i < 10; i++) {
        genResults.push(((await clusterGen.next()).value as SearchResult));
        // This one can see duplicates for some prefixes due to some paths having outbound
        // paths of lower total cost.
        pathsResults.push(((await pathsGen.next()).value as SearchResult));
      }
