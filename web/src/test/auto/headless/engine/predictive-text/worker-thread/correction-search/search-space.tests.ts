...
import { correction, getBestMatches, models, SearchPath, SearchSpace } from '@keymanapp/lm-worker/test-index';
...
const cantTestSetup = () => {
  ...
  // c + an
  // ca + n
  const c3_t2_space = new SearchSpace([c_an_path, ca_n_path]);
  ...
  const c3_t1_space = new SearchSpace([rootSpace.addInput([
  ...
  const c2_t2_space = new SearchSpace([c_a_path, ca_dl1_path]);
  ...
  // [cam, can] + "
  const c4_t2_space = new SearchSpace([c3_t1_space.addInput([
  ...
  const c4_t3_space = new SearchSpace([c_an_q_path, c_a_nq_path, can_q_dl1q_path]);
  const c_an_q_t_path = c4_t3_space.addInput([
    {sample: {insert: "t", deleteLeft: 0}, p: 0.9}
  ], 0.9);

  // c + a + n
  const c3_t3_space = new SearchSpace([c2_t2_space.addInput([
  ...
  const c5_4t_space = new SearchSpace([c_an_q_t_path, c_a_n_qt_path]);
...
describe('SearchSpace + SearchPath', () => {
  ...
  it('Simple search (paralleling "Small integration test")', async () => {
    ...
    await checkRepeatableResults_teh(iter, searchSpace3.spaceId);
  });
  ...
  it('Allows reiteration (sequentially)', async () => {
    ...
    await checkRepeatableResults_teh(iter2, searchSpace3.spaceId);
  });
  ...
  describe("Splitting", () => {
    describe("on SearchPath", () => {
      it('handles simple cases built from two-char transforms', () => {
        ...
        for(let entry of entries) {
          path = new SearchSpace([path]).addInput([{sample: { insert: entry, deleteLeft: 0 }, p: .5}], .5);
        }
      ...
      it('splits inputs with delete properly when needed - char index 0', () => {
        ...
        path = new SearchSpace([path]).addInput([{
    ...
    describe('on SearchSpace', () => {
      it('handles cases built solely from single-char transforms', () => {
        let rootSpace = new SearchSpace(testModel);
        const inputs = [...'apples'];

        let space = rootSpace;
        let lSpace: SearchSpace;
        for(const input of inputs) {
          space = new SearchSpace([space.addInput([{sample: {insert: input, deleteLeft: 0}, p: 0.5}], .5)]);
      ...
      it('handles cases built from two paths', () => {
        ...
        space1 = new SearchSpace([space1.addInput([{sample: {insert: 'al', deleteLeft: 0}, p: 0.5}], .5)]);
        let path1 = space1.addInput([{sample: { insert: 'e', deleteLeft: 0}, p: 0.5}], .5);

        let space2 = rootSpace;
        space2 = new SearchSpace([space2.addInput([{sample: {insert: 'a', deleteLeft: 0}, p: 0.5}], .5)]);
        ...
        let combinedSpace = new SearchSpace([path1, path2]);
