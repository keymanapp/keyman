...
import { analyzePathMergesAndSplits, assembleTransforms, buildEdgeWindow, ContextToken, ContextTokenization, EditOperation, EditTuple, ExtendedEditOperation, models, PendingTokenization, SearchPath, traceInsertEdits } from '@keymanapp/lm-worker/test-index';
...
describe('ContextTokenization', function() {
  ...
  describe('evaluateTransition', () => {
    it('handles simple case - new whitespace + new empty token', () => {
      ...
      assert.deepEqual(
        (tokenization.tokens[tokenization.tokens.length - 2].searchSpace as SearchPath).lastInput,
    ...
    it('handles simple case - new character added to last token', () => {
      ...
      assert.deepEqual(tokenization.tail.searchSpace.parents, [baseTokenization.tail.searchSpace]);
      assert.equal(
    ...
    it('properly manages empty token re-inserted by complex transform', () => {
      ...
      for(let i of inputTransformMap.keys()) {
        ...
        assert.deepEqual((tokenization.tokens[tailIndex + i].searchSpace as SearchPath).lastInput,
    ...
    it('handles large delete + insert transforms properly', () => {
      ...
      const boundaryTailInput = boundaryToken.sourceIdentifiers[boundaryToken.sourceIdentifiers.length - 1];
      ...
      assert.deepEqual(tokenization.tokens[tokenization.tokens.length-2].sourceIdentifiers, [{segment: {trueTransform: inputTransform, transitionId: inputTransform.id, start: 0}, bestProbFromSet: 1}]);
      assert.deepEqual(tokenization.tokens[tokenization.tokens.length-1].sourceIdentifiers, [{segment: {trueTransform: inputTransform, transitionId: inputTransform.id, start: 1}, bestProbFromSet: 1}]);
