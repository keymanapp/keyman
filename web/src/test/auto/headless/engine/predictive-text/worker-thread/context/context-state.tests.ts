...
describe('ContextState', () => {
  ...
  describe('analyzeTransition', function() {
    ...
    it("properly matches and aligns when a 'wordbreak' is added", function() {
      ...
      assert.equal(finalTokenization.tokens[finalTokenization.tokens.length - 2].searchSpace.inputCount, 1);
      // empty transform
      assert.equal(finalTokenization.tokens[finalTokenization.tokens.length - 1].searchSpace.inputCount, 1);
      assert.isTrue(finalTokenization.tail.searchSpace instanceof SearchPath);
      assert.deepEqual((finalTokenization.tail.searchSpace as SearchPath).lastInput, [{sample: { insert: '', deleteLeft: 0 }, p: 1}]);
    });
    ...
    it("properly matches and aligns when whitespace before final empty token is extended", function() {
      ...
      assert.equal(finalTokenization.tail.searchSpace.inputCount, 1);
      assert.isTrue(finalTokenization.tail.searchSpace instanceof SearchPath);
      assert.deepEqual((finalTokenization.tail.searchSpace as SearchPath).lastInput, [{sample: { insert: '', deleteLeft: 0 }, p: 1}]);
    });
    ...
    it("properly matches and aligns when lead token is removed AND a 'wordbreak' is added'", function() {
      ...
      assert.equal(finalTokenization.tokens[finalTokenization.tokens.length - 2].searchSpace.inputCount, 1);
      assert.equal(
        finalTokenization.tokens[finalTokenization.tokens.length - 1].searchSpace.inputCount, 1
      );
    });
