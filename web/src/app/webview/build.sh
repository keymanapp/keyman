...
compile_and_copy() {
  ...
  cp "${KEYMAN_ROOT}/web/build/engine/core-processor/obj/import/core/"km-core.{mjs,wasm} "${KEYMAN_ROOT}/web/build/app/webview/debug/"
  cp "${KEYMAN_ROOT}/web/build/engine/core-processor/obj/import/core/"km-core.{mjs,wasm} "${KEYMAN_ROOT}/web/build/app/webview/release/"

  # Clean the sourcemaps of .. and . components
  for script in "$KEYMAN_ROOT/web/build/$SUBPROJECT_NAME/debug/"*.js; do
    if [[ "${script}" == *"/km-core.mjs" ]]; then
      continue
    fi
