...
import { type TextStore, type ProcessorAction } from "keyman/engine/keyboard";
...
export class PredictionContext extends EventEmitter<PredictionContextEventMap> {
  ...
  private readonly suggestionApplier: (suggestion: Suggestion, processorAction?: ProcessorAction) => ProcessorAction;
   ...
   */
  private acceptInternal(suggestion: Suggestion, processorAction?: ProcessorAction): ProcessorAction {
    if(!suggestion) {
      return null;
    }
    ...
    if(suggestion.tag == 'revert') {
      ...
      return this.suggestionApplier(suggestion, processorAction);
    }
  }
   ...
   * @param processorAction When set with the results of applying an incoming
   ...
   * return a ProcessorAction.  Else returns null.ß
   */
  public accept(suggestion: Suggestion, processorAction?: ProcessorAction): ProcessorAction {
    // Selecting a suggestion or a reversion should both clear selection
    // and clear the reversion-displaying state of the banner.
    this.selected = null;

    const results = this.acceptInternal(suggestion, processorAction);

    // By default, we assume we were triggered by the banner.
    this._recentAcceptCause = processorAction ? 'key' : 'banner';
