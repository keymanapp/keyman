...
import { type ProcessorAction, type TextStore } from "keyman/engine/keyboard";
...
export class PredictionContext extends EventEmitter<PredictionContextEventMap> {
  ...
  private readonly suggestionApplier: (suggestion: Suggestion, ruleBehavior?: ProcessorAction) => ProcessorAction;
  ...
  public triggerPredictions() {
    const outputTarget = this.currentTextStore;
    if(!outputTarget) {
      return Promise.resolve([]);
    }
   ...
   */
  private acceptInternal(suggestion: Suggestion, ruleBehavior?: ProcessorAction): ProcessorAction {
    if(!suggestion) {
      return null;
    }
   ...
   */
  public accept(suggestion: Suggestion, ruleBehavior?: ProcessorAction): ProcessorAction {
