...
export class JSKeyboardProcessor extends EventEmitter<EventMap> {
    ...
    if(nothingDeletable && keyEvent.Lcode == Codes.keyCodes.K_BKSP && matchBehavior.triggerKeyDefault) {
      ...
      if(defaultBehavior) {
        if(!matchBehavior) {
          ...
          this.mergeInDefaults(matchBehavior, defaultBehavior);
        }
  ...
  private performNewContextEvent(outputTarget: OutputTargetBase): RuleBehavior {
    ...
    if (ruleBehavior) {
      this.finalize(ruleBehavior, outputTarget);
    }
    return ruleBehavior;
  }
  ...
  public finalize(data: RuleBehavior, outputTarget: OutputTargetInterface): void {
   ...
   * @param other
   */
  public mergeInDefaults(first: RuleBehavior, other: RuleBehavior): void {
