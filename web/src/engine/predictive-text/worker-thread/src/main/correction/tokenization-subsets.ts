 ...
 */
export interface TokenizationEdge {
  /**
  ...
  /**
   * A unique identifier associated with this TokenizationEdge and its
...
export interface TokenizationSubset {
   ...
   * application.
   */
  readonly transitionPaths: Map<ContextTokenization, TokenizationEdge>;
}
...
export class TokenizationSubsetBuilder {
  ...
  addPrecomputation(tokenization: ContextTokenization, precomputation: TokenizationTransitionEdits, p: number) {
    ...
    const forTokenization: TokenizationEdge = entry.transitionPaths.get(tokenization) ?? {
