 ...
 */
export function determineContextSlideDeltas(srcContext: Context, dstContext: Context): {
  prefix: string,
  dropCnt: number
} {
  ...
  return {
    ...
    prefix: rawDelta > 0 ? dst.slice(0, rawDelta) : '',
    dropCnt: rawDelta < 0 ? KMWString.length(src.slice(0, -rawDelta)) : 0
  }
}