...
export class SearchSpace implements SearchBatcher {
  ...
  private _split(
    charIndex: number,
    model: LexicalModel,
    splitCache: Map<number, {head: SearchSpace, tail: SearchSpace} []>
    ...
    if(this.codepointLength == charIndex) {
      ...
      return [[this, new SearchSpace(model)]];
    }

    // Ensure common split-ancestors still resolve to the same entity.
    const componentPaths = this.selectionQueue.toArray();
    let baseResultSet: [SearchSpace, SearchSpace][] = [];

    const deduplicateSplitResults = (results: [SearchSpace, SearchSpace][]) => {
      // Re-merge paths that converge to the same point.
      const duplicateMap: Map<number, [SearchSpace, SearchSpace][]> = new Map();
      results.forEach(result => {
        const headSpaceId = result[0].spaceId;
        const arr: [SearchSpace, SearchSpace][] = duplicateMap.get(headSpaceId) ?? [];
        arr.push(result);
        duplicateMap.set(result[0].spaceId, arr);
      });

      const finalResults: [SearchSpace, SearchSpace][] = [];
      for(const splits of duplicateMap.values()) {
        ...
        const tailSpace = new SearchSpace(paths);
    ...
    if(pathFiltering.inParent.length > 0) {
      const parentResults = pathFiltering.inParent.flatMap((path) => {
        ...
        const results = path.parents[0]._split(charIndex, model, splitCache);

        return results.map((results) => {
          const tailSpace = new SearchSpace([results[1].addInput([...path.inputs], path.bestProbInEdge)]);
          results[1] = tailSpace;
          return results;
        });
      });
    ...
    const pathResults: [SearchSpace, SearchSpace][] = pathFiltering.inCurrent.map((path) => {
      const parentSpace = path.parents[0] ?? new SearchSpace(model);
      ...
      if(path.codepointLength - path.edgeLength == charIndex) {
        ...
        const newPath = new SearchSpace(model).addInput([...path.inputs], path.bestProbInEdge);
        return [parentSpace, new SearchSpace([newPath])];
      } else {
        ...
        const newHeadSpace = pathSplitCacheArray[pathCharIndex]?.head ?? new SearchSpace([parentSpace.addInput([...results[0].inputs], path.bestProbInEdge)]);
        const newTailSpace = pathSplitCacheArray[pathCharIndex]?.tail ?? new SearchSpace([new SearchSpace(model).addInput([...results[1].inputs], path.bestProbInEdge)]);
