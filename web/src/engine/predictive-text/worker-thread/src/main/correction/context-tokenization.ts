...
import { TokenizationEdge } from './tokenization-subsets.js';
...
export class ContextTokenization {
   ...
   */
  readonly transitionEdits?: TokenizationEdge;
  ...
  constructor(tokens: ContextToken[], alignment: TokenizationEdge, taillessTrueKeystroke: Transform);
  constructor(
    param1: ContextToken[] | ContextTokenization,
    alignment?: TokenizationEdge,
   ...
   * @param tokenizationEdge Batched results from one or more
   ...
   * TokenizationEdge to be built.
   * @returns
   */
  evaluateTransition(
    tokenizationEdge: TokenizationEdge,
    lexicalModel: LexicalModel,
    sourceInput: Transform,
    bestProbFromSet: number
  ): ContextTokenization {
    const { alignment: alignment, inputs } = tokenizationEdge;
    ...
    return new ContextTokenization(
      ...
      null /* tokenMapping */,
      determineTaillessTrueKeystroke(tokenizationEdge)
    );
  }
}
 ...
 * @returns
 */
export function determineTaillessTrueKeystroke(tokenizationAnalysis: TokenizationEdge) {
