...
export class ModelCompositor {
  ...
  acceptSuggestion(suggestion: Suggestion, context: Context, postTransform?: Transform): Reversion {
    ...
    if(this.contextTracker) {
      ...
      let acceptedContext = models.applyTransform(fullTransform, context);
      ...
      appliedTransition.replaceFinal(applicationState, originalTransition.inputDistribution, originalTransition.preservationTransform);
      appliedTransition.final.appliedSuggestionId = suggestion.id;
      appliedTransition.final.appliedInput = originalTransition.final.appliedInput;
      appliedTransition.final.suggestions = originalTransition.final.suggestions;
  ...
  async applyReversion(reversion: Reversion, context: Context): Promise<Suggestion[]> {
    ...
    this.contextTracker.cache.clearNewerThanKey(-reversion.transformId);
